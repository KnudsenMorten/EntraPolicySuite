<#
.SYNOPSIS
    Entra-ID-User-Tagging-Troubleshooting - engine script in the Entra-Policy-Suite solution.

.NOTES
    Solution       : Entra-Policy-Suite
    File           : Entra-ID-User-Tagging-Troubleshooting.ps1
...
<#
    ...
    # Testing Only 
        ...
        $EntraID_Users_ALL_Scoped = $EntraID_Users_ALL | where-Object { ($_.DisplayName -ne "On-Premises Directory Synchronization Service Account") -and `
                                                                        ($_.UserPrincipalName -like "Afregning9060*") }
