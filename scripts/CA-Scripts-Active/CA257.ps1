<#
.SYNOPSIS
    CA257 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA257
# Policy Name           : CA257-Users-Internal-Developers-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload
# Policy Persona Target : Users-Internal-Developerss
# -------------------------------------------------------------------------------------------------------------------------------------
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA257-Users-Internal-Developers-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA257-Users-Internal-Developers-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA257-Users-Internal-Developers-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA257-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
                   -GroupType DynamicMembership `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -startsWith "Internal_Developer") and (user.extensionAttribute8 -contains "_pilot1")' `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA257-Users-Internal-Developers-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA257-Users-Internal-Developers-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA257-Users-Internal-Developers" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA257-Users-Internal-Developers-Pilot1-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-Users-Internal-Developers-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-Users-Internal-Developers" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
                       -SecurityEnabled $true `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-Users-Internal-Developers-Pilot1-Assigned"]
    }

    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA257-Users-Internal-Developers-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA257-Initial" `
                  -DisplayName "CA257-Initial-Users-Internal-Developers-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload" `
                  ...
                  -SC_CloudAppSecurity_IsEnabled $true `
                  -SC_CloudAppSecurity_CloudAppSecurity_Type "blockDownloads" `
                  -State disabled `
                  -CreateOnly
}
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA257-Users-Internal-Developers-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA257-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-Users-Internal-Developers-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA257-Initial", "CA257-Pilot1", "CA257-Pilot2", "CA257-Pilot3", "CA257-Prod") `
                  -DisplayName "CA257-Pilot1-Users-Internal-Developers-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA257-Users-Internal-Developers-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -startsWith "Internal_Developer") and ((user.extensionAttribute8 -contains "_pilot1") or (user.extensionAttribute8 -contains "_pilot2"))' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA257-Users-Internal-Developers-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA257-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA257-Users-Internal-Developers-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {

        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-Users-Internal-Developers-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-Users-Internal-Developers-Pilot2-Assigned"]
    }

    # Change to pilot 2 configuration, look for any policies using prefix array
    EntraCAPolicy -CAPolicyPrefixArray @("CA257-Initial", "CA257-Pilot1", "CA257-Pilot2", "CA257-Pilot3", "CA257-Prod") `
                  -DisplayName "CA257-Pilot2-Users-Internal-Developers-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA257-Users-Internal-Developers-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -startsWith "Internal_Developer") and ((user.extensionAttribute8 -contains "_pilot1") or (user.extensionAttribute8 -contains "_pilot2") or (user.extensionAttribute8 -contains "_pilot3"))' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA257-Users-Internal-Developers-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA257-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA257-Users-Internal-Developers-Pilot3-Assigned"]

    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {

        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-Users-Internal-Developers-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-Users-Internal-Developers-Pilot3-Assigned"]
    }

    # Change to pilot 3 configuration, look for any policies using prefix array
    EntraCAPolicy -CAPolicyPrefixArray @("CA257-Initial", "CA257-Pilot1", "CA257-Pilot2", "CA257-Pilot3", "CA257-Prod") `
                  -DisplayName "CA257-Pilot3-Users-Internal-Developers-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        $Users_Internal_Developers_All     = $EntraGroupsHashTable["Entra-CA-Users-Internal-Developers-All-Dynamic"]
        $Users_Internal_Developers_MFA_All = $EntraGroupsHashTable["Entra-CA-Users-Internal-Developers-Req-MFA-All-Dynamic"]
        ...
        EntraGroup -DisplayName "Entra-CA-CA257-Users-Internal-Developers-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    if ( ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") -or ($Group_Targeting_Method -eq "Manual_Assignment_Simple") ) {
        ...
        $Users_Internal_Developers_All     = $EntraGroupsHashTable["Entra-CA-Users-Internal-Developers-All-Assigned"]
        $Users_Internal_Developers_MFA_All = $EntraGroupsHashTable["Entra-CA-Users-Internal-Developers-Req-MFA-All-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA257-Initial", "CA257-Pilot1", "CA257-Pilot2", "CA257-Pilot3", "CA257-Prod") `
                  -DisplayName "CA257-Prod-Users-Internal-Developers-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA257-$($PolicyVersion)" `
                  -DisplayName "CA257-$($PolicyVersion)-Users-Internal-Developers-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA257-Prod" `
                  -SC_CloudAppSecurity_IsEnabled $true `
                  -SC_CloudAppSecurity_CloudAppSecurity_Type "blockDownloads" `
                  -CreateUpdate
