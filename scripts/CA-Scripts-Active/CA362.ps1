<#
.SYNOPSIS
    CA362 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA362
# Policy Name           : CA362-Users-External-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA362-Users-External-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA362-Users-External-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA362-Users-External-Developers-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA362-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
                   -GroupType DynamicMembership `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -startsWith "External_Admin") and (user.extensionAttribute8 -contains "_pilot1")' `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA362-Users-External-Developers-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA362-Users-External-Developers-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA362-Admins" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA362-Users-External-Developers-Pilot1-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-Users-External-Developers-Pilot$pilot-Assigned" `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-Users-External-Developers-Pilot1-Assigned"]
    }

    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA362-Users-External-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA362-Initial" `
                  -DisplayName "CA362-Initial-Users-External-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA362-Users-External-Developers-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA362-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-Users-External-Developers-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA362-Initial", "CA362-Pilot1", "CA362-Pilot2", "CA362-Pilot3", "CA362-Prod") `
                  -DisplayName "CA362-Pilot1-Users-External-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA362-Users-External-Developers-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -startsWith "External_Admin") and ((user.extensionAttribute8 -contains "_pilot1") or (user.extensionAttribute8 -contains "_pilot2"))' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA362-Users-External-Developers-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA362-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA362-Users-External-Developers-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {

        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-Users-External-Developers-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-Users-External-Developers-Pilot2-Assigned"]
    }

    # Change to pilot 2 configuration, look for any policies using prefix array
    EntraCAPolicy -CAPolicyPrefixArray @("CA362-Initial", "CA362-Pilot1", "CA362-Pilot2", "CA362-Pilot3", "CA362-Prod") `
                  -DisplayName "CA362-Pilot2-Users-External-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA362-Users-External-Developers-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -startsWith "External_Admin") and ((user.extensionAttribute8 -contains "_pilot1") or (user.extensionAttribute8 -contains "_pilot2") or (user.extensionAttribute8 -contains "_pilot3"))' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA362-Users-External-Developers-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA362-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA362-Users-External-Developers-Pilot3-Assigned"]

    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {

        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-Users-External-Developers-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-Users-External-Developers-Pilot3-Assigned"]
    }

    # Change to pilot 3 configuration, look for any policies using prefix array
    EntraCAPolicy -CAPolicyPrefixArray @("CA362-Initial", "CA362-Pilot1", "CA362-Pilot2", "CA362-Pilot3", "CA362-Prod") `
                  -DisplayName "CA362-Pilot3-Users-External-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        $Admins_External_All           = $EntraGroupsHashTable["Entra-CA-Users-External-Developers-All-Dynamic"]
        $Admins_External_MFA_All       = $EntraGroupsHashTable["Entra-CA-Users-External-Developers-Req-MFA-All-Dynamic"]
        $Admins_External_FIDO_All      = $EntraGroupsHashTable["Entra-CA-Users-External-Developers-Req-FIDO-All-Dynamic"]
        ...
        EntraGroup -DisplayName "Entra-CA-CA362-Users-External-Developers-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    if ( ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") -or ($Group_Targeting_Method -eq "Manual_Assignment_Simple") ) {
        ...
        $Admins_External_All           = $EntraGroupsHashTable["Entra-CA-Users-External-Developers-All-Assigned"]
        $Admins_External_MFA_All       = $EntraGroupsHashTable["Entra-CA-Users-External-Developers-Req-MFA-All-Assigned"]
        $Admins_External_FIDO_All      = $EntraGroupsHashTable["Entra-CA-Users-External-Developers-Req-FIDO-All-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA362-Initial", "CA362-Pilot1", "CA362-Pilot2", "CA362-Pilot3", "CA362-Prod") `
                  -DisplayName "CA362-Prod-Users-External-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($Users_External_Developers_All.id)) `
                  -State enabled `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA362-$($PolicyVersion)" `
                  -DisplayName "CA362-$($PolicyVersion)-Users-External-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA362-Prod" `
                  -SC_SignInFrequency_Value 1 `
                  -SC_SignInFrequency_Type "days" `
                  -SC_SignInFrequency_AuthenticationType "primaryAndSecondaryAuthentication" `
                  -SC_SignInFrequency_FrequencyInterval "timeBased" `
