<#
.SYNOPSIS
    CA207 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA207
# Policy Name           : CA207-Users-Internal-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA207-Users-Internal-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA207-Users-Internal-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA207-Users-Internal-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA207-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA207-Users-Internal-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA207-Users-Internal-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA207-Users-Internal" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA207-Users-Internal-Pilot1-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
        foreach ($pilot in 1..3) {
            ...
            EntraGroup -DisplayName "Entra-CA-Users-Internal-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-Users-Internal" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
                       -SecurityEnabled $true `
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA207-Users-Internal-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA207-Initial" `
                  -DisplayName "CA207-Initial-Users-Internal-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA207-Users-Internal-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA207-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA207-Initial", "CA207-Pilot1", "CA207-Pilot2", "CA207-Pilot3", "CA207-Prod") `
                  -DisplayName "CA207-Pilot1-Users-Internal-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA207-Users-Internal-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA207-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA207-Users-Internal-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA207-Initial", "CA207-Pilot1", "CA207-Pilot2", "CA207-Pilot3", "CA207-Prod") `
                  -DisplayName "CA207-Pilot2-Users-Internal-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA207-Users-Internal-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA207-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA207-Users-Internal-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA207-Initial", "CA207-Pilot1", "CA207-Pilot2", "CA207-Pilot3", "CA207-Prod") `
                  -DisplayName "CA207-Pilot3-Users-Internal-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA207-Users-Internal-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA207-Initial", "CA207-Pilot1", "CA207-Pilot2", "CA207-Pilot3", "CA207-Prod") `
                  -DisplayName "CA207-Prod-Users-Internal-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA207-$($PolicyVersion)" `
                  -DisplayName "CA207-$($PolicyVersion)-Users-Internal-Office365-AnyPlatform-Unmanaged-AppEnforcedRestrictions-BlockDownload" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA207-Prod" `
                  -SC_CloudAppSecurity_IsEnabled $true `
                  -SC_CloudAppSecurity_CloudAppSecurity_Type "blockDownloads" `
                  -CreateUpdate
