<#
.SYNOPSIS
    CA301 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA301
# Policy Name           : CA301-Users-External-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA301-Users-External-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA301-Users-External-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA301-Users-External-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA301-Users-External" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
                   -GroupType DynamicMembership `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["External_User_Req_MFA_Pilot1"])' `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA301-Users-External-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA301-Users-External-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA301-Users-External" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA301-Users-External-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA301-Users-External-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA301-Initial" `
                  -DisplayName "CA301-Initial-Users-External-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA301-Users-External-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA301-Users-External" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA301-Initial", "CA301-Pilot1", "CA301-Pilot2", "CA301-Pilot3", "CA301-Prod") `
                  -DisplayName "CA301-Pilot1-Users-External-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA301-Users-External-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["External_User_Req_MFA_Pilot1","External_User_Req_MFA_Pilot2"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA301-Users-External-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA301-Users-External" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA301-Users-External-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA301-Initial", "CA301-Pilot1", "CA301-Pilot2", "CA301-Pilot3", "CA301-Prod") `
                  -DisplayName "CA301-Pilot2-Users-External-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA301-Users-External-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["External_User_Req_MFA_Pilot1","External_User_Req_MFA_Pilot2","External_User_Req_MFA_Pilot3"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA301-Users-External-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA301-Users-External" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA301-Users-External-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA301-Initial", "CA301-Pilot1", "CA301-Pilot2", "CA301-Pilot3", "CA301-Prod") `
                  -DisplayName "CA301-Pilot3-Users-External-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA301-Users-External-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA301-Initial", "CA301-Pilot1", "CA301-Pilot2", "CA301-Pilot3", "CA301-Prod") `
                  -DisplayName "CA301-Prod-Users-External-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA301-$($PolicyVersion)" `
                  -DisplayName "CA301-$($PolicyVersion)-Users-External-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA301-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls "mfa" `
                  -CreateUpdate
