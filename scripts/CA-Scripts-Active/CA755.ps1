<#
.SYNOPSIS
    CA755 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA755
# Policy Name           : CA755-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-FIDO-Allow
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA755-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-FIDO-Allow-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA755-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-FIDO-Allow" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA755-AppSystem-Test-Users-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA755-AppSystem-Test-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
                   -GroupType DynamicMembership `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["AppSystem_Test_User_Req_FIDO_Pilot1"])' `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA755-AppSystem-Test-Users-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA755-AppSystem-Test-Users-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA755-AppSystem-Test-Users" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA755-AppSystem-Test-Users-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA755-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-FIDO-Allow-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA755-Initial" `
                  -DisplayName "CA755-Initial-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-FIDO-Allow" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA755-AppSystem-Test-Users-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA755-AppSystem-Test-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA755-Initial", "CA755-Pilot1", "CA755-Pilot2", "CA755-Pilot3", "CA755-Prod") `
                  -DisplayName "CA755-Pilot1-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-FIDO-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA755-AppSystem-Test-Users-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["AppSystem_Test_User_Req_FIDO_Pilot1","AppSystem_Test_User_Req_FIDO_Pilot2"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA755-AppSystem-Test-Users-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA755-AppSystem-Test-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA755-AppSystem-Test-Users-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA755-Initial", "CA755-Pilot1", "CA755-Pilot2", "CA755-Pilot3", "CA755-Prod") `
                  -DisplayName "CA755-Pilot2-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-FIDO-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA755-AppSystem-Test-Users-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["AppSystem_Test_User_Req_FIDO_Pilot1","AppSystem_Test_User_Req_FIDO_Pilot2","AppSystem_Test_User_Req_FIDO_Pilot3"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA755-AppSystem-Test-Users-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA755-AppSystem-Test-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA755-AppSystem-Test-Users-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA755-Initial", "CA755-Pilot1", "CA755-Pilot2", "CA755-Pilot3", "CA755-Prod") `
                  -DisplayName "CA755-Pilot3-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-FIDO-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA755-AppSystem-Test-Users-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA755-Initial", "CA755-Pilot1", "CA755-Pilot2", "CA755-Pilot3", "CA755-Prod") `
                  -DisplayName "CA755-Prod-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-FIDO-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($AppSystem_Test_Users_FIDO_All.id)) `
                  -State enabled `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA755-$($PolicyVersion)" `
                  -DisplayName "CA755-$($PolicyVersion)-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-FIDO-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA755-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls @() `
                  -GC_authenticationStrength "FIDO Security Key & TAP" `
                  -CreateUpdate
