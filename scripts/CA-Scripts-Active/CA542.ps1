<#
.SYNOPSIS
    CA542 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA542
# Policy Name           : CA542-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-Pwd-Allow
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA542-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-Pwd-Allow-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA542-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA542-Teams-Rooms-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA542-Teams-Rooms" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
                   -GroupType DynamicMembership `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Teams_Room_Req_Pwd_Pilot1"])' `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA542-Teams-Rooms-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA542-Teams-Rooms-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA542-Teams-Rooms" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA542-Teams-Rooms-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA542-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-Pwd-Allow-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA542-Initial" `
                  -DisplayName "CA542-Initial-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA542-Teams-Rooms-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA542-Teams-Rooms" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA542-Initial", "CA542-Pilot1", "CA542-Pilot2", "CA542-Pilot3", "CA542-Prod") `
                  -DisplayName "CA542-Pilot1-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA542-Teams-Rooms-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Teams_Room_Req_Pwd_Pilot1","Teams_Room_Req_Pwd_Pilot2"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA542-Teams-Rooms-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA542-Teams-Rooms" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA542-Teams-Rooms-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA542-Initial", "CA542-Pilot1", "CA542-Pilot2", "CA542-Pilot3", "CA542-Prod") `
                  -DisplayName "CA542-Pilot2-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA542-Teams-Rooms-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Teams_Room_Req_Pwd_Pilot1","Teams_Room_Req_Pwd_Pilot2","Teams_Room_Req_Pwd_Pilot3"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA542-Teams-Rooms-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA542-Teams-Rooms" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA542-Teams-Rooms-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA542-Initial", "CA542-Pilot1", "CA542-Pilot2", "CA542-Pilot3", "CA542-Prod") `
                  -DisplayName "CA542-Pilot3-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA542-Teams-Rooms-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA542-Initial", "CA542-Pilot1", "CA542-Pilot2", "CA542-Pilot3", "CA542-Prod") `
                  -DisplayName "CA542-Prod-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($Teams_Rooms_Pwd_All.id)) `
                  -State enabled `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA542-$($PolicyVersion)" `
                  -DisplayName "CA542-$($PolicyVersion)-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA542-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls "mfa" `
                  -CreateUpdate
