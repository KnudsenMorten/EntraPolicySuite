<#
.SYNOPSIS
    CA262 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA262
# Policy Name           : CA262-Users-Internal-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA262-Users-Internal-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA262-Users-Internal-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA262-Users-Internal-Developers-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA262-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA262-Users-Internal-Developers-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA262-Users-Internal-Developers-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA262-Admins" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA262-Users-Internal-Developers-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA262-Users-Internal-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA262-Initial" `
                  -DisplayName "CA262-Initial-Users-Internal-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA262-Users-Internal-Developers-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA262-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA262-Initial", "CA262-Pilot1", "CA262-Pilot2", "CA262-Pilot3", "CA262-Prod") `
                  -DisplayName "CA262-Pilot1-Users-Internal-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA262-Users-Internal-Developers-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA262-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA262-Users-Internal-Developers-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA262-Initial", "CA262-Pilot1", "CA262-Pilot2", "CA262-Pilot3", "CA262-Prod") `
                  -DisplayName "CA262-Pilot2-Users-Internal-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA262-Users-Internal-Developers-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA262-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA262-Users-Internal-Developers-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA262-Initial", "CA262-Pilot1", "CA262-Pilot2", "CA262-Pilot3", "CA262-Prod") `
                  -DisplayName "CA262-Pilot3-Users-Internal-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        $Admins_Internal_All           = $EntraGroupsHashTable["Entra-CA-Users-Internal-Developers-All-Dynamic"]
        $Admins_Internal_MFA_All       = $EntraGroupsHashTable["Entra-CA-Users-Internal-Developers-Req-MFA-All-Dynamic"]
        $Admins_Internal_FIDO_All      = $EntraGroupsHashTable["Entra-CA-Users-Internal-Developers-Req-FIDO-All-Dynamic"]
        ...
        EntraGroup -DisplayName "Entra-CA-CA262-Users-Internal-Developers-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    if ( ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") -or ($Group_Targeting_Method -eq "Manual_Assignment_Simple") ) {
        ...
        $Admins_Internal_All           = $EntraGroupsHashTable["Entra-CA-Users-Internal-Developers-All-Assigned"]
        $Admins_Internal_MFA_All       = $EntraGroupsHashTable["Entra-CA-Users-Internal-Developers-Req-MFA-All-Assigned"]
        $Admins_Internal_FIDO_All      = $EntraGroupsHashTable["Entra-CA-Users-Internal-Developers-Req-FIDO-All-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA262-Initial", "CA262-Pilot1", "CA262-Pilot2", "CA262-Pilot3", "CA262-Prod") `
                  -DisplayName "CA262-Prod-Users-Internal-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA262-$($PolicyVersion)" `
                  -DisplayName "CA262-$($PolicyVersion)-Users-Internal-Developers-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA262-Prod" `
                  -SC_SignInFrequency_Value 7 `
                  -SC_SignInFrequency_Type "days" `
                  -SC_SignInFrequency_AuthenticationType "primaryAndSecondaryAuthentication" `
                  -SC_SignInFrequency_FrequencyInterval "timeBased" `
