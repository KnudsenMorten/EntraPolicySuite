<#
.SYNOPSIS
    CA205 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA205
# Policy Name           : CA205-Users-Internal-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA205-Users-Internal-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA205-Users-Internal-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA205-Users-Internal-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA205-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA205-Users-Internal-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA205-Users-Internal-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA205-Users-Internal" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA205-Users-Internal-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA205-Users-Internal-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA205-Initial" `
                  -DisplayName "CA205-Initial-Users-Internal-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA205-Users-Internal-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA205-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA205-Initial", "CA205-Pilot1", "CA205-Pilot2", "CA205-Pilot3", "CA205-Prod") `
                  -DisplayName "CA205-Pilot1-Users-Internal-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA205-Users-Internal-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA205-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA205-Users-Internal-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA205-Initial", "CA205-Pilot1", "CA205-Pilot2", "CA205-Pilot3", "CA205-Prod") `
                  -DisplayName "CA205-Pilot2-Users-Internal-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA205-Users-Internal-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA205-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA205-Users-Internal-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA205-Initial", "CA205-Pilot1", "CA205-Pilot2", "CA205-Pilot3", "CA205-Prod") `
                  -DisplayName "CA205-Pilot3-Users-Internal-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA205-Users-Internal-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA205-Initial", "CA205-Pilot1", "CA205-Pilot2", "CA205-Pilot3", "CA205-Prod") `
                  -DisplayName "CA205-Prod-Users-Internal-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA205-$($PolicyVersion)" `
                  -DisplayName "CA205-$($PolicyVersion)-Users-Internal-AllApps-AnyPlatform-MediumHighSigninRisk-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA205-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls "mfa" `
                  -CreateUpdate
