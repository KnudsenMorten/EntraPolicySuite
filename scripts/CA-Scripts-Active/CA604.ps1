<#
.SYNOPSIS
    CA604 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA604
# Policy Name           : CA604-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-FIDO-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA604-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-FIDO-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA604-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-FIDO-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA604-ServiceAccounts-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA604-ServiceAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA604-ServiceAccounts-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA604-ServiceAccounts-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA604-ServiceAccounts" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA604-ServiceAccounts-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA604-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-FIDO-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA604-Initial" `
                  -DisplayName "CA604-Initial-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-FIDO-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA604-ServiceAccounts-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA604-ServiceAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA604-Initial", "CA604-Pilot1", "CA604-Pilot2", "CA604-Pilot3", "CA604-Prod") `
                  -DisplayName "CA604-Pilot1-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-FIDO-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA604-ServiceAccounts-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA604-ServiceAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA604-ServiceAccounts-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA604-Initial", "CA604-Pilot1", "CA604-Pilot2", "CA604-Pilot3", "CA604-Prod") `
                  -DisplayName "CA604-Pilot2-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-FIDO-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA604-ServiceAccounts-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA604-ServiceAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA604-ServiceAccounts-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA604-Initial", "CA604-Pilot1", "CA604-Pilot2", "CA604-Pilot3", "CA604-Prod") `
                  -DisplayName "CA604-Pilot3-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-FIDO-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA604-ServiceAccounts-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA604-Initial", "CA604-Pilot1", "CA604-Pilot2", "CA604-Pilot3", "CA604-Prod") `
                  -DisplayName "CA604-Prod-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-FIDO-Enforce" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA604-$($PolicyVersion)" `
                  -DisplayName "CA604-$($PolicyVersion)-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-FIDO-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA604-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls @() `
                  -GC_authenticationStrength "FIDO Security Key & TAP" `
                  -CreateUpdate
