<#
.SYNOPSIS
    CA211 - engine script in the Entra-Policy-Suite solution.
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA211-Users-Internal-Office365-InsiderRiskElevated-Block-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA211-Users-Internal-Office365-InsiderRiskElevated-Block" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA211-Users-Internal-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA211-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA211-Users-Internal-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA211-Users-Internal-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA211-Users-Internal" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA211-Users-Internal-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA211-Users-Internal-Office365-InsiderRiskElevated-Block-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA211-Initial" `
                  -DisplayName "CA211-Initial-Users-Internal-Office365-InsiderRiskElevated-Block" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA211-Users-Internal-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA211-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA211-Initial", "CA211-Pilot1", "CA211-Pilot2", "CA211-Pilot3", "CA211-Prod") `
                  -DisplayName "CA211-Pilot1-Users-Internal-Office365-InsiderRiskElevated-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA211-Users-Internal-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA211-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA211-Users-Internal-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA211-Initial", "CA211-Pilot1", "CA211-Pilot2", "CA211-Pilot3", "CA211-Prod") `
                  -DisplayName "CA211-Pilot2-Users-Internal-Office365-InsiderRiskElevated-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA211-Users-Internal-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA211-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA211-Users-Internal-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA211-Initial", "CA211-Pilot1", "CA211-Pilot2", "CA211-Pilot3", "CA211-Prod") `
                  -DisplayName "CA211-Pilot3-Users-Internal-Office365-InsiderRiskElevated-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA211-Users-Internal-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA211-Initial", "CA211-Pilot1", "CA211-Pilot2", "CA211-Pilot3", "CA211-Prod") `
                  -DisplayName "CA211-Prod-Users-Internal-Office365-InsiderRiskElevated-Block" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA211-$($PolicyVersion)" `
                  -DisplayName "CA211-$($PolicyVersion)-Users-Internal-Office365-InsiderRiskElevated-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA211-Prod" `
                  -GC_Operator "OR" `
