<#
.SYNOPSIS
    CA547 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA547
# Policy Name           : CA547-Teams-Rooms-AllApps-TrustedLocations-WHfB-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA547-Teams-Rooms-AllApps-TrustedLocations-WHfB-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA547-Teams-Rooms-AllApps-TrustedLocations-WHfB-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA547-Teams-Rooms-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA547-Teams-Rooms" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA547-Teams-Rooms-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA547-Teams-Rooms-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA547-Teams-Rooms" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA547-Teams-Rooms-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA547-Teams-Rooms-AllApps-TrustedLocations-WHfB-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA547-Initial" `
                  -DisplayName "CA547-Initial-Teams-Rooms-AllApps-TrustedLocations-WHfB-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA547-Teams-Rooms-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA547-Teams-Rooms" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA547-Initial", "CA547-Pilot1", "CA547-Pilot2", "CA547-Pilot3", "CA547-Prod") `
                  -DisplayName "CA547-Pilot1-Teams-Rooms-AllApps-TrustedLocations-WHfB-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA547-Teams-Rooms-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA547-Teams-Rooms" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA547-Teams-Rooms-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA547-Initial", "CA547-Pilot1", "CA547-Pilot2", "CA547-Pilot3", "CA547-Prod") `
                  -DisplayName "CA547-Pilot2-Teams-Rooms-AllApps-TrustedLocations-WHfB-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA547-Teams-Rooms-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA547-Teams-Rooms" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA547-Teams-Rooms-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA547-Initial", "CA547-Pilot1", "CA547-Pilot2", "CA547-Pilot3", "CA547-Prod") `
                  -DisplayName "CA547-Pilot3-Teams-Rooms-AllApps-TrustedLocations-WHfB-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA547-Teams-Rooms-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA547-Initial", "CA547-Pilot1", "CA547-Pilot2", "CA547-Pilot3", "CA547-Prod") `
                  -DisplayName "CA547-Prod-Teams-Rooms-AllApps-TrustedLocations-WHfB-Enforce" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA547-$($PolicyVersion)" `
                  -DisplayName "CA547-$($PolicyVersion)-Teams-Rooms-AllApps-TrustedLocations-WHfB-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA547-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls @() `
                  -GC_authenticationStrength "WHfB + TAP" `
                  -CreateUpdate
