<#
.SYNOPSIS
    CA553 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA553
# Policy Name           : CA553-Shared-Mail-Users-RegisterSecurityInfo-TrustedLocations-MFA-Allow
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA553-Shared-Mail-Users-RegisterSecurityInfo-TrustedLocations-MFA-Allow-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA553-Shared-Mail-Users-RegisterSecurityInfo-TrustedLocations-MFA-Allow" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA553-Shared-Mail-Users-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA553-Shared-Mail-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
                   -GroupType DynamicMembership `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Shared_Mail_User_Req_MFA_Pilot1"])' `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA553-Shared-Mail-Users-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA553-Shared-Mail-Users-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA553-Shared-Mail-Users" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA553-Shared-Mail-Users-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA553-Shared-Mail-Users-RegisterSecurityInfo-TrustedLocations-MFA-Allow-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA553-Initial" `
                  -DisplayName "CA553-Initial-Shared-Mail-Users-RegisterSecurityInfo-TrustedLocations-MFA-Allow" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA553-Shared-Mail-Users-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA553-Shared-Mail-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA553-Initial", "CA553-Pilot1", "CA553-Pilot2", "CA553-Pilot3", "CA553-Prod") `
                  -DisplayName "CA553-Pilot1-Shared-Mail-Users-RegisterSecurityInfo-TrustedLocations-MFA-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA553-Shared-Mail-Users-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Shared_Mail_User_Req_MFA_Pilot1","Shared_Mail_User_Req_MFA_Pilot2"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA553-Shared-Mail-Users-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA553-Shared-Mail-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA553-Shared-Mail-Users-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA553-Initial", "CA553-Pilot1", "CA553-Pilot2", "CA553-Pilot3", "CA553-Prod") `
                  -DisplayName "CA553-Pilot2-Shared-Mail-Users-RegisterSecurityInfo-TrustedLocations-MFA-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA553-Shared-Mail-Users-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Shared_Mail_User_Req_MFA_Pilot1","Shared_Mail_User_Req_MFA_Pilot2","Shared_Mail_User_Req_MFA_Pilot3"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA553-Shared-Mail-Users-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA553-Shared-Mail-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA553-Shared-Mail-Users-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA553-Initial", "CA553-Pilot1", "CA553-Pilot2", "CA553-Pilot3", "CA553-Prod") `
                  -DisplayName "CA553-Pilot3-Shared-Mail-Users-RegisterSecurityInfo-TrustedLocations-MFA-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA553-Shared-Mail-Users-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA553-Initial", "CA553-Pilot1", "CA553-Pilot2", "CA553-Pilot3", "CA553-Prod") `
                  -DisplayName "CA553-Prod-Shared-Mail-Users-RegisterSecurityInfo-TrustedLocations-MFA-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($Shared_Mail_Users_MFA_All.id)) `
                  -State enabled `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA553-$($PolicyVersion)" `
                  -DisplayName "CA553-$($PolicyVersion)-Shared-Mail-Users-RegisterSecurityInfo-TrustedLocations-MFA-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA553-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls "mfa" `
                  -CreateUpdate
