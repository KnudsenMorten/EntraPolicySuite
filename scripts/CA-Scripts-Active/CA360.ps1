<#
.SYNOPSIS
    CA360 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA360
# Policy Name           : CA360-Users-External-Developers-EntraGSA-Windows-MFACompliant-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA360-Users-External-Developers-EntraGSA-Windows-MFACompliant-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA360-Users-External-Developers-EntraGSA-Windows-MFACompliant-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA360-Users-External-Developers-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA360-Users-External-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA360-Users-External-Developers-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA360-Users-External-Developers-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA360-Users-External-Developers" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA360-Users-External-Developers-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA360-Users-External-Developers-EntraGSA-Windows-MFACompliant-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA360-Initial" `
                  -DisplayName "CA360-Initial-Users-External-Developers-EntraGSA-Windows-MFACompliant-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA360-Users-External-Developers-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA360-Users-External-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA360-Initial", "CA360-Pilot1", "CA360-Pilot2", "CA360-Pilot3", "CA360-Prod") `
                  -DisplayName "CA360-Pilot1-Users-External-Developers-EntraGSA-Windows-MFACompliant-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA360-Users-External-Developers-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA360-Users-External-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA360-Users-External-Developers-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA360-Initial", "CA360-Pilot1", "CA360-Pilot2", "CA360-Pilot3", "CA360-Prod") `
                  -DisplayName "CA360-Pilot2-Users-External-Developers-EntraGSA-Windows-MFACompliant-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA360-Users-External-Developers-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA360-Users-External-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA360-Users-External-Developers-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA360-Initial", "CA360-Pilot1", "CA360-Pilot2", "CA360-Pilot3", "CA360-Prod") `
                  -DisplayName "CA360-Pilot3-Users-External-Developers-EntraGSA-Windows-MFACompliant-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA360-Users-External-Developers-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA360-Initial", "CA360-Pilot1", "CA360-Pilot2", "CA360-Pilot3", "CA360-Prod") `
                  -DisplayName "CA360-Prod-Users-External-Developers-EntraGSA-Windows-MFACompliant-Enforce" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA360-$($PolicyVersion)" `
                  -DisplayName "CA360-$($PolicyVersion)-Users-External-Developers-EntraGSA-Windows-MFACompliant-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA360-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls "mfa", "compliantDevice" `
                  -CreateUpdate
