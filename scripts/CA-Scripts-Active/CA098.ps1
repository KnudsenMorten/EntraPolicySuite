<#
.SYNOPSIS
    CA098 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA098
# Policy Name           : CA098-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-FIDO-Allow
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA098-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-FIDO-Allow-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA098-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-FIDO-Allow" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA098-BreakGlassAccounts-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA098-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
                   -GroupType DynamicMembership `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Break_Glass_Account_Req_FIDO_Pilot1"])' `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA098-BreakGlassAccounts-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA098-BreakGlassAccounts-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA098-BreakGlassAccounts" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA098-BreakGlassAccounts-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA098-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-FIDO-Allow-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA098-Initial" `
                  -DisplayName "CA098-Initial-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-FIDO-Allow" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA098-BreakGlassAccounts-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA098-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA098-Initial", "CA098-Pilot1", "CA098-Pilot2", "CA098-Pilot3", "CA098-Prod") `
                  -DisplayName "CA098-Pilot1-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-FIDO-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA098-BreakGlassAccounts-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Break_Glass_Account_Req_FIDO_Pilot1","Break_Glass_Account_Req_FIDO_Pilot2"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA098-BreakGlassAccounts-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA098-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA098-BreakGlassAccounts-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA098-Initial", "CA098-Pilot1", "CA098-Pilot2", "CA098-Pilot3", "CA098-Prod") `
                  -DisplayName "CA098-Pilot2-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-FIDO-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA098-BreakGlassAccounts-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Break_Glass_Account_Req_FIDO_Pilot1","Break_Glass_Account_Req_FIDO_Pilot2","Break_Glass_Account_Req_FIDO_Pilot3"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA098-BreakGlassAccounts-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA098-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA098-BreakGlassAccounts-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA098-Initial", "CA098-Pilot1", "CA098-Pilot2", "CA098-Pilot3", "CA098-Prod") `
                  -DisplayName "CA098-Pilot3-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-FIDO-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA098-Initial", "CA098-Pilot1", "CA098-Pilot2", "CA098-Pilot3", "CA098-Prod") `
                  -DisplayName "CA098-Prod-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-FIDO-Allow" `
                  -Cond_Users_IncludeUsers @(($BreakGlassAccounts_FIDO.id)) `
                  -Cond_Users_IncludeGroups @(($BreakGlassAccounts_FIDO_All.id)) `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA098-$($PolicyVersion)" `
                  -DisplayName "CA098-$($PolicyVersion)-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-FIDO-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA098-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls @() `
                  -GC_authenticationStrength "FIDO Security Key & TAP" `
                  -CreateUpdate
