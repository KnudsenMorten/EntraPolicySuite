<#
.SYNOPSIS
    CA259 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA259
# Policy Name           : CA259-Users-Internal-Developers-SelectedApps-AnyPlatform-Block
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA259-Users-Internal-Developers-SelectedApps-AnyPlatform-Block-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA259-Users-Internal-Developers-SelectedApps-AnyPlatform-Block" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA259-Users-Internal-Developers-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA259-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA259-Users-Internal-Developers-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA259-Users-Internal-Developers-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA259-Users-Internal-Developers" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA259-Users-Internal-Developers-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA259-Users-Internal-Developers-SelectedApps-AnyPlatform-Block-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA259-Initial" `
                  -DisplayName "CA259-Initial-Users-Internal-Developers-SelectedApps-AnyPlatform-Block" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA259-Users-Internal-Developers-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA259-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA259-Initial", "CA259-Pilot1", "CA259-Pilot2", "CA259-Pilot3", "CA259-Prod") `
                  -DisplayName "CA259-Pilot1-Users-Internal-Developers-SelectedApps-AnyPlatform-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA259-Users-Internal-Developers-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA259-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA259-Users-Internal-Developers-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA259-Initial", "CA259-Pilot1", "CA259-Pilot2", "CA259-Pilot3", "CA259-Prod") `
                  -DisplayName "CA259-Pilot2-Users-Internal-Developers-SelectedApps-AnyPlatform-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA259-Users-Internal-Developers-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA259-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA259-Users-Internal-Developers-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA259-Initial", "CA259-Pilot1", "CA259-Pilot2", "CA259-Pilot3", "CA259-Prod") `
                  -DisplayName "CA259-Pilot3-Users-Internal-Developers-SelectedApps-AnyPlatform-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA259-Users-Internal-Developers-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA259-Initial", "CA259-Pilot1", "CA259-Pilot2", "CA259-Pilot3", "CA259-Prod") `
                  -DisplayName "CA259-Prod-Users-Internal-Developers-SelectedApps-AnyPlatform-Block" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA259-$($PolicyVersion)" `
                  -DisplayName "CA259-$($PolicyVersion)-Users-Internal-Developers-SelectedApps-AnyPlatform-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA259-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls "block" `
                  -CreateUpdate
