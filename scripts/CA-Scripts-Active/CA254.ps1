<#
.SYNOPSIS
    CA254 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA254
# Policy Name           : CA254-Users-Internal-Developers-AllApps-WindowsMacOS-UnmanagedDevices-SigninFrequency-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA254-Users-Internal-Developers-AllApps-WindowsMacOS-UnmanagedDevices-SigninFrequency-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA254-Users-Internal-Developers-AllApps-WindowsMacOS-UnmanagedDevices-SigninFrequency-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA254-Users-Internal-Developers-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA254-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA254-Users-Internal-Developers-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA254-Users-Internal-Developers-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA254-Users-Internal-Developers" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA254-Users-Internal-Developers-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA254-Users-Internal-Developers-AllApps-WindowsMacOS-UnmanagedDevices-SigninFrequency-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA254-Initial" `
                  -DisplayName "CA254-Initial-Users-Internal-Developers-AllApps-WindowsMacOS-UnmanagedDevices-SigninFrequency-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA254-Users-Internal-Developers-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA254-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA254-Initial", "CA254-Pilot1", "CA254-Pilot2", "CA254-Pilot3", "CA254-Prod") `
                  -DisplayName "CA254-Pilot1-Users-Internal-Developers-AllApps-WindowsMacOS-UnmanagedDevices-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA254-Users-Internal-Developers-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA254-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA254-Users-Internal-Developers-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA254-Initial", "CA254-Pilot1", "CA254-Pilot2", "CA254-Pilot3", "CA254-Prod") `
                  -DisplayName "CA254-Pilot2-Users-Internal-Developers-AllApps-WindowsMacOS-UnmanagedDevices-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA254-Users-Internal-Developers-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA254-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA254-Users-Internal-Developers-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA254-Initial", "CA254-Pilot1", "CA254-Pilot2", "CA254-Pilot3", "CA254-Prod") `
                  -DisplayName "CA254-Pilot3-Users-Internal-Developers-AllApps-WindowsMacOS-UnmanagedDevices-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA254-Users-Internal-Developers-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA254-Initial", "CA254-Pilot1", "CA254-Pilot2", "CA254-Pilot3", "CA254-Prod") `
                  -DisplayName "CA254-Prod-Users-Internal-Developers-AllApps-WindowsMacOS-UnmanagedDevices-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA254-$($PolicyVersion)" `
                  -DisplayName "CA254-$($PolicyVersion)-Users-Internal-Developers-AllApps-WindowsMacOS-UnmanagedDevices-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA254-Prod" `
                  -SC_SignInFrequency_Value 1 `
                  -SC_SignInFrequency_Type "days" `
                  -SC_SignInFrequency_AuthenticationType "primaryAndSecondaryAuthentication" `
                  -SC_SignInFrequency_FrequencyInterval "timeBased" `
