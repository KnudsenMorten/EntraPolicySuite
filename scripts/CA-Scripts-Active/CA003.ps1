<#
.SYNOPSIS
    CA003 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA003
# Policy Name           : CA003-Global-AllApps-AnyPlatform-LegacyAuthentication-ExchangeActiveSyncClients-Block
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA003-Global-AllApps-AnyPlatform-LegacyAuthentication-ExchangeActiveSyncClients-Block-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA003-Global-AllApps-AnyPlatform-LegacyAuthentication-ExchangeActiveSyncClients-Block" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA003-Global-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA003-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA003-Global-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA003-Global-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA003-Global" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA003-Global-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA003-Global-AllApps-AnyPlatform-LegacyAuthentication-ExchangeActiveSyncClients-Block-Excluded-Assigned"]
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA003-Global-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA003-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA003-Initial", "CA003-Pilot1", "CA003-Pilot2", "CA003-Pilot3", "CA003-Prod") `
                  -DisplayName "CA003-Pilot1-Global-AllApps-AnyPlatform-LegacyAuthentication-ExchangeActiveSyncClients-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA003-Global-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA003-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA003-Global-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA003-Initial", "CA003-Pilot1", "CA003-Pilot2", "CA003-Pilot3", "CA003-Prod") `
                  -DisplayName "CA003-Pilot2-Global-AllApps-AnyPlatform-LegacyAuthentication-ExchangeActiveSyncClients-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA003-Global-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA003-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA003-Global-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA003-Initial", "CA003-Pilot1", "CA003-Pilot2", "CA003-Pilot3", "CA003-Prod") `
                  -DisplayName "CA003-Pilot3-Global-AllApps-AnyPlatform-LegacyAuthentication-ExchangeActiveSyncClients-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA003-Global-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA003-Initial", "CA003-Pilot1", "CA003-Pilot2", "CA003-Pilot3", "CA003-Prod") `
                  -DisplayName "CA003-Prod-Global-AllApps-AnyPlatform-LegacyAuthentication-ExchangeActiveSyncClients-Block" `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA003-$($PolicyVersion)" `
                  -DisplayName "CA003-$($PolicyVersion)-Global-AllApps-AnyPlatform-LegacyAuthentication-ExchangeActiveSyncClients-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA003-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls @("Block") `
                  -CreateUpdate
