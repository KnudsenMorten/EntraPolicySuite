<#
.SYNOPSIS
    CA007 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA007
# Policy Name           : CA007-Global-AllApps-AnyPlatform-UnknownPlatforms-Block
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA007-Global-AllApps-AnyPlatform-UnknownPlatforms-Block-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA007-Global-AllApps-AnyPlatform-UnknownPlatforms-Block" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA007-Global-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA007-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA007-Global-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA007-Global-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA007-Global" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA007-Global-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA007-Global-AllApps-AnyPlatform-UnknownPlatforms-Block-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA007-Initial" `
                  -DisplayName "CA007-Initial-Global-AllApps-AnyPlatform-UnknownPlatforms-Block" `
                  ...
                  -Cond_Platforms_excludePlatforms @("android", "iOS", "macOS", "windows", "linux","windowsPhone") `
                  -GC_Operator "OR" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA007-Global-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA007-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA007-Initial", "CA007-Pilot1", "CA007-Pilot2", "CA007-Pilot3", "CA007-Prod") `
                  -DisplayName "CA007-Pilot1-Global-AllApps-AnyPlatform-UnknownPlatforms-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA007-Global-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA007-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA007-Global-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA007-Initial", "CA007-Pilot1", "CA007-Pilot2", "CA007-Pilot3", "CA007-Prod") `
                  -DisplayName "CA007-Pilot2-Global-AllApps-AnyPlatform-UnknownPlatforms-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA007-Global-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA007-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA007-Global-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA007-Initial", "CA007-Pilot1", "CA007-Pilot2", "CA007-Pilot3", "CA007-Prod") `
                  -DisplayName "CA007-Pilot3-Global-AllApps-AnyPlatform-UnknownPlatforms-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA007-Global-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA007-Initial", "CA007-Pilot1", "CA007-Pilot2", "CA007-Pilot3", "CA007-Prod") `
                  -DisplayName "CA007-Prod-Global-AllApps-AnyPlatform-UnknownPlatforms-Block" `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA007-$($PolicyVersion)" `
                  -DisplayName "CA007-$($PolicyVersion)-Global-AllApps-AnyPlatform-UnknownPlatforms-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
                  -Cond_App_includeApplications @("All") `
                  -Cond_Platforms_includePlatforms @("All") `
                  -Cond_Platforms_excludePlatforms @("android", "iOS", "macOS", "windows", "linux") `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls @("Block") `
                  -State disabled `
                  -CreateOnly
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA007-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls @("Block") `
                  -CreateUpdate
