<#
.SYNOPSIS
    CA501 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA501
# Policy Name           : CA501-Shared-Device-Users-AllApps-AnyPlatform-SigninFrequency-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA501-Shared-Device-Users-AllApps-AnyPlatform-SigninFrequency-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA501-Shared-Device-Users-AllApps-AnyPlatform-SigninFrequency-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA501-Shared-Device-Users-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA501-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA501-Shared-Device-Users-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA501-Shared-Device-Users-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA501-Shared-Device-Users" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA501-Shared-Device-Users-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA501-Shared-Device-Users-AllApps-AnyPlatform-SigninFrequency-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA501-Initial" `
                  -DisplayName "CA501-Initial-Shared-Device-Users-AllApps-AnyPlatform-SigninFrequency-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA501-Shared-Device-Users-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA501-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA501-Initial", "CA501-Pilot1", "CA501-Pilot2", "CA501-Pilot3", "CA501-Prod") `
                  -DisplayName "CA501-Pilot1-Shared-Device-Users-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA501-Shared-Device-Users-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA501-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA501-Shared-Device-Users-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA501-Initial", "CA501-Pilot1", "CA501-Pilot2", "CA501-Pilot3", "CA501-Prod") `
                  -DisplayName "CA501-Pilot2-Shared-Device-Users-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA501-Shared-Device-Users-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA501-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA501-Shared-Device-Users-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA501-Initial", "CA501-Pilot1", "CA501-Pilot2", "CA501-Pilot3", "CA501-Prod") `
                  -DisplayName "CA501-Pilot3-Shared-Device-Users-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA501-Shared-Device-Users-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA501-Initial", "CA501-Pilot1", "CA501-Pilot2", "CA501-Pilot3", "CA501-Prod") `
                  -DisplayName "CA501-Prod-Shared-Device-Users-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA501-$($PolicyVersion)" `
                  -DisplayName "CA501-$($PolicyVersion)-Shared-Device-Users-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA501-Prod" `
                  -SC_SignInFrequency_Value 1 `
                  -SC_SignInFrequency_Type "days" `
                  -SC_SignInFrequency_AuthenticationType "primaryAndSecondaryAuthentication" `
                  -SC_SignInFrequency_FrequencyInterval "timeBased" `
