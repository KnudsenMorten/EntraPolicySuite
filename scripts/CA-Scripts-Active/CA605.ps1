<#
.SYNOPSIS
    CA605 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA605
# Policy Name           : CA605-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA605-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA605-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA605-ServiceAccounts-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA605-ServiceAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA605-ServiceAccounts-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA605-ServiceAccounts-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA605-ServiceAccounts" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA605-ServiceAccounts-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA605-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA605-Initial" `
                  -DisplayName "CA605-Initial-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA605-ServiceAccounts-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA605-ServiceAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA605-Initial", "CA605-Pilot1", "CA605-Pilot2", "CA605-Pilot3", "CA605-Prod") `
                  -DisplayName "CA605-Pilot1-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA605-ServiceAccounts-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA605-ServiceAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA605-ServiceAccounts-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA605-Initial", "CA605-Pilot1", "CA605-Pilot2", "CA605-Pilot3", "CA605-Prod") `
                  -DisplayName "CA605-Pilot2-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA605-ServiceAccounts-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA605-ServiceAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA605-ServiceAccounts-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA605-Initial", "CA605-Pilot1", "CA605-Pilot2", "CA605-Pilot3", "CA605-Prod") `
                  -DisplayName "CA605-Pilot3-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA605-ServiceAccounts-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA605-Initial", "CA605-Pilot1", "CA605-Pilot2", "CA605-Pilot3", "CA605-Prod") `
                  -DisplayName "CA605-Prod-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA605-$($PolicyVersion)" `
                  -DisplayName "CA605-$($PolicyVersion)-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA605-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls "mfa" `
                  -CreateUpdate
