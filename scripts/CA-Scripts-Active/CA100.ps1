<#
.SYNOPSIS
    CA100 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA100
# Policy Name           : CA100-Admins-Internal-AllApps-AnyPlatform-SigninFrequency-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA100-Admins-Internal-AllApps-AnyPlatform-SigninFrequency-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA100-Admins-Internal-AllApps-AnyPlatform-SigninFrequency-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA100-Admins-Internal-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA100-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA100-Admins-Internal-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA100-Admins-Internal-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA100-Admins" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA100-Admins-Internal-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA100-Admins-Internal-AllApps-AnyPlatform-SigninFrequency-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA100-Initial" `
                  -DisplayName "CA100-Initial-Admins-Internal-AllApps-AnyPlatform-SigninFrequency-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA100-Admins-Internal-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA100-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA100-Initial", "CA100-Pilot1", "CA100-Pilot2", "CA100-Pilot3", "CA100-Prod") `
                  -DisplayName "CA100-Pilot1-Admins-Internal-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA100-Admins-Internal-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA100-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA100-Admins-Internal-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA100-Initial", "CA100-Pilot1", "CA100-Pilot2", "CA100-Pilot3", "CA100-Prod") `
                  -DisplayName "CA100-Pilot2-Admins-Internal-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA100-Admins-Internal-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA100-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA100-Admins-Internal-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA100-Initial", "CA100-Pilot1", "CA100-Pilot2", "CA100-Pilot3", "CA100-Prod") `
                  -DisplayName "CA100-Pilot3-Admins-Internal-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA100-Admins-Internal-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA100-Initial", "CA100-Pilot1", "CA100-Pilot2", "CA100-Pilot3", "CA100-Prod") `
                  -DisplayName "CA100-Prod-Admins-Internal-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA100-$($PolicyVersion)" `
                  -DisplayName "CA100-$($PolicyVersion)-Admins-Internal-AllApps-AnyPlatform-SigninFrequency-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA100-Prod" `
                  -SC_SignInFrequency_Value 12 `
                  -SC_SignInFrequency_Type "hours" `
                  -SC_SignInFrequency_AuthenticationType "primaryAndSecondaryAuthentication" `
                  -SC_SignInFrequency_FrequencyInterval "timeBased" `
