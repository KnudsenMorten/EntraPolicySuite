<#
.SYNOPSIS
    CA201 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA201
# Policy Name           : CA201-Users-Internal-AllApps-AnyPlatform-PersistentBrowser-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA201-Users-Internal-AllApps-AnyPlatform-PersistentBrowser-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA201-Users-Internal-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA201-Users-Internal-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA201-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA201-Users-Internal-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA201-Users-Internal-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA201-Users-Internal" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA201-Users-Internal-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA201-Users-Internal-AllApps-AnyPlatform-PersistentBrowser-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA201-Initial" `
                  -DisplayName "CA201-Initial-Users-Internal-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA201-Users-Internal-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA201-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA201-Initial", "CA201-Pilot1", "CA201-Pilot2", "CA201-Pilot3", "CA201-Prod") `
                  -DisplayName "CA201-Pilot1-Users-Internal-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA201-Users-Internal-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA201-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA201-Users-Internal-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA201-Initial", "CA201-Pilot1", "CA201-Pilot2", "CA201-Pilot3", "CA201-Prod") `
                  -DisplayName "CA201-Pilot2-Users-Internal-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA201-Users-Internal-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA201-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA201-Users-Internal-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA201-Initial", "CA201-Pilot1", "CA201-Pilot2", "CA201-Pilot3", "CA201-Prod") `
                  -DisplayName "CA201-Pilot3-Users-Internal-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA201-Users-Internal-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA201-Initial", "CA201-Pilot1", "CA201-Pilot2", "CA201-Pilot3", "CA201-Prod") `
                  -DisplayName "CA201-Prod-Users-Internal-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA201-$($PolicyVersion)" `
                  -DisplayName "CA201-$($PolicyVersion)-Users-Internal-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA201-Prod" `
                  -SC_persistentBrowser_Mode "never" `
                  -SC_persistentBrowser_isEnabled $true `
                  -CreateUpdate
