<#
.SYNOPSIS
    CA006 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA006
# Policy Name           : CA006-Global-AllApps-AnyPlatform-HighUserRisk-Block
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA006-Global-AllApps-AnyPlatform-HighUserRisk-Block-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA006-Global-AllApps-AnyPlatform-HighUserRisk-Block" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA006-Global-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA006-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA006-Global-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA006-Global-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA006-Global" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA006-Global-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA006-Global-AllApps-AnyPlatform-HighUserRisk-Block-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA006-Initial" `
                  -DisplayName "CA006-Initial-Global-AllApps-AnyPlatform-HighUserRisk-Block" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA006-Global-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA006-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA006-Initial", "CA006-Pilot1", "CA006-Pilot2", "CA006-Pilot3", "CA006-Prod") `
                  -DisplayName "CA006-Pilot1-Global-AllApps-AnyPlatform-HighUserRisk-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA006-Global-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA006-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA006-Global-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA006-Initial", "CA006-Pilot1", "CA006-Pilot2", "CA006-Pilot3", "CA006-Prod") `
                  -DisplayName "CA006-Pilot2-Global-AllApps-AnyPlatform-HighUserRisk-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA006-Global-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA006-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA006-Global-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA006-Initial", "CA006-Pilot1", "CA006-Pilot2", "CA006-Pilot3", "CA006-Prod") `
                  -DisplayName "CA006-Pilot3-Global-AllApps-AnyPlatform-HighUserRisk-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA006-Global-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA006-Initial", "CA006-Pilot1", "CA006-Pilot2", "CA006-Pilot3", "CA006-Prod") `
                  -DisplayName "CA006-Prod-Global-AllApps-AnyPlatform-HighUserRisk-Block" `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA006-$($PolicyVersion)" `
                  -DisplayName "CA006-$($PolicyVersion)-Global-AllApps-AnyPlatform-HighUserRisk-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA006-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls @("Block") `
                  -CreateUpdate
