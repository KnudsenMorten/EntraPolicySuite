<#
.SYNOPSIS
    CA543 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA543
# Policy Name           : CA543-Teams-Rooms-AllApps-TrustedLocations-Pwd-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA543-Teams-Rooms-AllApps-TrustedLocations-Pwd-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA543-Teams-Rooms-AllApps-TrustedLocations-Pwd-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA543-Teams-Rooms-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA543-Teams-Rooms" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA543-Teams-Rooms-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA543-Teams-Rooms-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA543-Teams-Rooms" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA543-Teams-Rooms-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA543-Teams-Rooms-AllApps-TrustedLocations-Pwd-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA543-Initial" `
                  -DisplayName "CA543-Initial-Teams-Rooms-AllApps-TrustedLocations-Pwd-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA543-Teams-Rooms-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA543-Teams-Rooms" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA543-Initial", "CA543-Pilot1", "CA543-Pilot2", "CA543-Pilot3", "CA543-Prod") `
                  -DisplayName "CA543-Pilot1-Teams-Rooms-AllApps-TrustedLocations-Pwd-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA543-Teams-Rooms-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA543-Teams-Rooms" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA543-Teams-Rooms-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA543-Initial", "CA543-Pilot1", "CA543-Pilot2", "CA543-Pilot3", "CA543-Prod") `
                  -DisplayName "CA543-Pilot2-Teams-Rooms-AllApps-TrustedLocations-Pwd-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA543-Teams-Rooms-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA543-Teams-Rooms" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA543-Teams-Rooms-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA543-Initial", "CA543-Pilot1", "CA543-Pilot2", "CA543-Pilot3", "CA543-Prod") `
                  -DisplayName "CA543-Pilot3-Teams-Rooms-AllApps-TrustedLocations-Pwd-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA543-Teams-Rooms-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA543-Initial", "CA543-Pilot1", "CA543-Pilot2", "CA543-Pilot3", "CA543-Prod") `
                  -DisplayName "CA543-Prod-Teams-Rooms-AllApps-TrustedLocations-Pwd-Enforce" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA543-$($PolicyVersion)" `
                  -DisplayName "CA543-$($PolicyVersion)-Teams-Rooms-AllApps-TrustedLocations-Pwd-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA543-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls "mfa" `
                  -CreateUpdate
