<#
.SYNOPSIS
    CA606 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA606
# Policy Name           : CA606-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-MFA-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA606-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-MFA-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA606-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-MFA-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA606-ServiceAccounts-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA606-ServiceAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA606-ServiceAccounts-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA606-ServiceAccounts-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA606-ServiceAccounts" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA606-ServiceAccounts-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA606-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-MFA-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA606-Initial" `
                  -DisplayName "CA606-Initial-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-MFA-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA606-ServiceAccounts-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA606-ServiceAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA606-Initial", "CA606-Pilot1", "CA606-Pilot2", "CA606-Pilot3", "CA606-Prod") `
                  -DisplayName "CA606-Pilot1-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA606-ServiceAccounts-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA606-ServiceAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA606-ServiceAccounts-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA606-Initial", "CA606-Pilot1", "CA606-Pilot2", "CA606-Pilot3", "CA606-Prod") `
                  -DisplayName "CA606-Pilot2-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA606-ServiceAccounts-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA606-ServiceAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA606-ServiceAccounts-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA606-Initial", "CA606-Pilot1", "CA606-Pilot2", "CA606-Pilot3", "CA606-Prod") `
                  -DisplayName "CA606-Pilot3-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA606-ServiceAccounts-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA606-Initial", "CA606-Pilot1", "CA606-Pilot2", "CA606-Pilot3", "CA606-Prod") `
                  -DisplayName "CA606-Prod-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA606-$($PolicyVersion)" `
                  -DisplayName "CA606-$($PolicyVersion)-ServiceAccounts-AllApps-AnyPlatform-TrustedLocations-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA606-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls @() `
                  -GC_authenticationStrength "WHfB + TAP" `
                  -CreateUpdate
