<#
.SYNOPSIS
    CA756 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA756
# Policy Name           : CA756-AppSystem-Test-Users-AllApps-TrustedLocations-FIDO-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA756-AppSystem-Test-Users-AllApps-TrustedLocations-FIDO-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA756-AppSystem-Test-Users-AllApps-TrustedLocations-FIDO-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA756-AppSystem-Test-Users-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA756-AppSystem-Test-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA756-AppSystem-Test-Users-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA756-AppSystem-Test-Users-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA756-AppSystem-Test-Users" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA756-AppSystem-Test-Users-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA756-AppSystem-Test-Users-AllApps-TrustedLocations-FIDO-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA756-Initial" `
                  -DisplayName "CA756-Initial-AppSystem-Test-Users-AllApps-TrustedLocations-FIDO-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA756-AppSystem-Test-Users-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA756-AppSystem-Test-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA756-Initial", "CA756-Pilot1", "CA756-Pilot2", "CA756-Pilot3", "CA756-Prod") `
                  -DisplayName "CA756-Pilot1-AppSystem-Test-Users-AllApps-TrustedLocations-FIDO-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA756-AppSystem-Test-Users-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA756-AppSystem-Test-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA756-AppSystem-Test-Users-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA756-Initial", "CA756-Pilot1", "CA756-Pilot2", "CA756-Pilot3", "CA756-Prod") `
                  -DisplayName "CA756-Pilot2-AppSystem-Test-Users-AllApps-TrustedLocations-FIDO-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA756-AppSystem-Test-Users-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA756-AppSystem-Test-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA756-AppSystem-Test-Users-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA756-Initial", "CA756-Pilot1", "CA756-Pilot2", "CA756-Pilot3", "CA756-Prod") `
                  -DisplayName "CA756-Pilot3-AppSystem-Test-Users-AllApps-TrustedLocations-FIDO-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA756-AppSystem-Test-Users-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA756-Initial", "CA756-Pilot1", "CA756-Pilot2", "CA756-Pilot3", "CA756-Prod") `
                  -DisplayName "CA756-Prod-AppSystem-Test-Users-AllApps-TrustedLocations-FIDO-Enforce" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA756-$($PolicyVersion)" `
                  -DisplayName "CA756-$($PolicyVersion)-AppSystem-Test-Users-AllApps-TrustedLocations-FIDO-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA756-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls @() `
                  -GC_authenticationStrength "FIDO Security Key & TAP" `
                  -CreateUpdate
