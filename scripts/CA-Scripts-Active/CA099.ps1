<#
.SYNOPSIS
    CA099 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA099
# Policy Name           : CA099-BreakGlassAccounts-AllApps-AnyPlatform-FIDO-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA099-BreakGlassAccounts-AllApps-AnyPlatform-FIDO-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA099-BreakGlassAccounts-AllApps-AnyPlatform-FIDO-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA099-BreakGlassAccounts-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA099-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA099-BreakGlassAccounts-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA099-BreakGlassAccounts-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA099-BreakGlassAccounts" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA099-BreakGlassAccounts-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA099-BreakGlassAccounts-AllApps-AnyPlatform-FIDO-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA099-Initial" `
                  -DisplayName "CA099-Initial-BreakGlassAccounts-AllApps-AnyPlatform-FIDO-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA099-BreakGlassAccounts-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA099-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA099-Initial", "CA099-Pilot1", "CA099-Pilot2", "CA099-Pilot3", "CA099-Prod") `
                  -DisplayName "CA099-Pilot1-BreakGlassAccounts-AllApps-AnyPlatform-FIDO-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA099-BreakGlassAccounts-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA099-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA099-BreakGlassAccounts-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA099-Initial", "CA099-Pilot1", "CA099-Pilot2", "CA099-Pilot3", "CA099-Prod") `
                  -DisplayName "CA099-Pilot2-BreakGlassAccounts-AllApps-AnyPlatform-FIDO-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA099-BreakGlassAccounts-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA099-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA099-BreakGlassAccounts-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA099-Initial", "CA099-Pilot1", "CA099-Pilot2", "CA099-Pilot3", "CA099-Prod") `
                  -DisplayName "CA099-Pilot3-BreakGlassAccounts-AllApps-AnyPlatform-FIDO-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA099-BreakGlassAccounts-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA099-Initial", "CA099-Pilot1", "CA099-Pilot2", "CA099-Pilot3", "CA099-Prod") `
                  -DisplayName "CA099-Prod-BreakGlassAccounts-AllApps-AnyPlatform-FIDO-Enforce" `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA099-$($PolicyVersion)" `
                  -DisplayName "CA099-$($PolicyVersion)-BreakGlassAccounts-AllApps-AnyPlatform-FIDO-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA099-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls @() `
                  -GC_authenticationStrength "FIDO Security Key & TAP" `
                  -CreateUpdate
