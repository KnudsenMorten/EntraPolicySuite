<#
.SYNOPSIS
    CA000 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA000
# Policy Name           : CA000-Global-AllUsers-AllApps-AnyPlatform-MFA-CatchNoTag
# Policy Persona Target : Global
# -------------------------------------------------------------------------------------------------------------------------------------
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA000-Global-AllUsers-AllApps-AnyPlatform-MFA-CatchNoTag-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA000-Global-AllUsers-AllApps-AnyPlatform-MFA-CatchNoTag" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA000-Global-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA000-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
                   -GroupType DynamicMembership `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -startsWith "Global") and (user.extensionAttribute8 -contains "_pilot1")' `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA000-Global-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA000-Global-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA000-Global" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA000-Global-Pilot1-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-Global-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-Global" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
                       -SecurityEnabled $true `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-Global-Pilot1-Assigned"]
    }
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        $Admins_MFA_All                = $EntraGroupsHashTable["Entra-CA-Admins-Req-MFA-All-Dynamic"]
        $Admins_FIDO_All               = $EntraGroupsHashTable["Entra-CA-Admins-Req-FIDO-All-Dynamic"]
    ...
    if ( ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") -or ($Group_Targeting_Method -eq "Manual_Assignment_Simple") ) {
        ...
        $Admins_MFA_All                = $EntraGroupsHashTable["Entra-CA-Admins-Req-MFA-All-Assigned"]
        $Admins_FIDO_All               = $EntraGroupsHashTable["Entra-CA-Admins-Req-FIDO-All-Assigned"]
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA000-Global-AllUsers-AllApps-AnyPlatform-MFA-CatchNoTag-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA000-Initial" `
                  -DisplayName "CA000-Initial-Global-AllUsers-AllApps-AnyPlatform-MFA-CatchNoTag" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id), ($Users_All.id), ($Admins_All.id), ($ServiceAccounts_All.id), ($Users_Non_Managed_All.id), ($ADConnect_All.id), ($Shared_Mail_Users_All.id), ($Shared_Device_Users_All.id), ($Teams_Rooms_All.id), ($AppSystem_Test_Users_All.id)) `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA000-Global-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA000-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-Global-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA000-Initial", "CA000-Pilot1", "CA000-Pilot2", "CA000-Pilot3", "CA000-Prod") `
                  -DisplayName "CA000-Pilot1-Global-AllUsers-AllApps-AnyPlatform-MFA-CatchNoTag" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA000-Global-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -startsWith "Global") and ((user.extensionAttribute8 -contains "_pilot1") or (user.extensionAttribute8 -contains "_pilot2"))' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA000-Global-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA000-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA000-Global-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {

        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-Global-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-Global-Pilot2-Assigned"]
    }

    # Change to pilot 2 configuration, look for any policies using prefix array
    EntraCAPolicy -CAPolicyPrefixArray @("CA000-Initial", "CA000-Pilot1", "CA000-Pilot2", "CA000-Pilot3", "CA000-Prod") `
                  -DisplayName "CA000-Pilot2-Global-AllUsers-AllApps-AnyPlatform-MFA-CatchNoTag" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA000-Global-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -startsWith "Global") and ((user.extensionAttribute8 -contains "_pilot1") or (user.extensionAttribute8 -contains "_pilot2") or (user.extensionAttribute8 -contains "_pilot3"))' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA000-Global-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA000-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA000-Global-Pilot3-Assigned"]

    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {

        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-Global-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-Global" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-Global-Pilot3-Assigned"]
    }

    # Change to pilot 3 configuration, look for any policies using prefix array
    EntraCAPolicy -CAPolicyPrefixArray @("CA000-Initial", "CA000-Pilot1", "CA000-Pilot2", "CA000-Pilot3", "CA000-Prod") `
                  -DisplayName "CA000-Pilot3-Global-AllUsers-AllApps-AnyPlatform-MFA-CatchNoTag" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA000-Initial", "CA000-Pilot1", "CA000-Pilot2", "CA000-Pilot3", "CA000-Prod") `
                  -DisplayName "CA000-Prod-Global-AllUsers-AllApps-AnyPlatform-MFA-CatchNoTag" `
                  -Cond_Users_IncludeUsers @("All") `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA000-$($PolicyVersion)" `
                  -DisplayName "CA000-$($PolicyVersion)-Global-CA000-Global-AllUsers-AllApps-AnyPlatform-MFA-CatchNoTag" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id), ($Users_All.id), ($Admins_All.id), ($ServiceAccounts_All.id), ($Users_Non_Managed_All.id), ($ADConnect_All.id), ($Shared_Mail_Users_All.id), ($Shared_Device_Users_All.id), ($Teams_Rooms_All.id), ($AppSystem_Test_Users_All.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA000-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls @("mfa") `
                  -CreateUpdate
}
