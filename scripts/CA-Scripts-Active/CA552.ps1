<#
.SYNOPSIS
    CA552 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA552
# Policy Name           : CA552-Shared-Mail-Users-AllApps-TrustedLocations-Pwd-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA552-Shared-Mail-Users-AllApps-TrustedLocations-Pwd-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA552-Shared-Mail-Users-AllApps-TrustedLocations-Pwd-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA552-Shared-Mail-Users-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA552-Shared-Mail-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA552-Shared-Mail-Users-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA552-Shared-Mail-Users-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA552-Shared-Mail-Users" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA552-Shared-Mail-Users-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA552-Shared-Mail-Users-AllApps-TrustedLocations-Pwd-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA552-Initial" `
                  -DisplayName "CA552-Initial-Shared-Mail-Users-AllApps-TrustedLocations-Pwd-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA552-Shared-Mail-Users-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA552-Shared-Mail-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA552-Initial", "CA552-Pilot1", "CA552-Pilot2", "CA552-Pilot3", "CA552-Prod") `
                  -DisplayName "CA552-Pilot1-Shared-Mail-Users-AllApps-TrustedLocations-Pwd-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA552-Shared-Mail-Users-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA552-Shared-Mail-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA552-Shared-Mail-Users-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA552-Initial", "CA552-Pilot1", "CA552-Pilot2", "CA552-Pilot3", "CA552-Prod") `
                  -DisplayName "CA552-Pilot2-Shared-Mail-Users-AllApps-TrustedLocations-Pwd-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA552-Shared-Mail-Users-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA552-Shared-Mail-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA552-Shared-Mail-Users-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA552-Initial", "CA552-Pilot1", "CA552-Pilot2", "CA552-Pilot3", "CA552-Prod") `
                  -DisplayName "CA552-Pilot3-Shared-Mail-Users-AllApps-TrustedLocations-Pwd-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA552-Shared-Mail-Users-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA552-Initial", "CA552-Pilot1", "CA552-Pilot2", "CA552-Pilot3", "CA552-Prod") `
                  -DisplayName "CA552-Prod-Shared-Mail-Users-AllApps-TrustedLocations-Pwd-Enforce" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA552-$($PolicyVersion)" `
                  -DisplayName "CA552-$($PolicyVersion)-Shared-Mail-Users-AllApps-TrustedLocations-Pwd-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA552-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls "mfa" `
                  -CreateUpdate
