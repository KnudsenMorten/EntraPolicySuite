<#
.SYNOPSIS
    CA751 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA751
# Policy Name           : CA751-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA751-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA751-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA751-AppSystem-Test-Users-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA751-AppSystem-Test-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
                   -GroupType DynamicMembership `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["AppSystem_Test_User_Req_Pwd_Pilot1"])' `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA751-AppSystem-Test-Users-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA751-AppSystem-Test-Users-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA751-AppSystem-Test-Users" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA751-AppSystem-Test-Users-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA751-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA751-Initial" `
                  -DisplayName "CA751-Initial-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA751-AppSystem-Test-Users-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA751-AppSystem-Test-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA751-Initial", "CA751-Pilot1", "CA751-Pilot2", "CA751-Pilot3", "CA751-Prod") `
                  -DisplayName "CA751-Pilot1-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA751-AppSystem-Test-Users-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["AppSystem_Test_User_Req_Pwd_Pilot1","AppSystem_Test_User_Req_Pwd_Pilot2"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA751-AppSystem-Test-Users-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA751-AppSystem-Test-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA751-AppSystem-Test-Users-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA751-Initial", "CA751-Pilot1", "CA751-Pilot2", "CA751-Pilot3", "CA751-Prod") `
                  -DisplayName "CA751-Pilot2-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA751-AppSystem-Test-Users-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["AppSystem_Test_User_Req_Pwd_Pilot1","AppSystem_Test_User_Req_Pwd_Pilot2","AppSystem_Test_User_Req_Pwd_Pilot3"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA751-AppSystem-Test-Users-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA751-AppSystem-Test-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA751-AppSystem-Test-Users-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA751-Initial", "CA751-Pilot1", "CA751-Pilot2", "CA751-Pilot3", "CA751-Prod") `
                  -DisplayName "CA751-Pilot3-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA751-AppSystem-Test-Users-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA751-Initial", "CA751-Pilot1", "CA751-Pilot2", "CA751-Pilot3", "CA751-Prod") `
                  -DisplayName "CA751-Prod-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($AppSystem_Test_Users_Pwd_All.id)) `
                  -State enabled `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA751-$($PolicyVersion)" `
                  -DisplayName "CA751-$($PolicyVersion)-AppSystem-Test-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA751-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls "mfa" `
                  -CreateUpdate
