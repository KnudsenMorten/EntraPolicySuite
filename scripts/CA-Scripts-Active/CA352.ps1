<#
.SYNOPSIS
    CA352 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA352
# Policy Name           : CA352-Users-External-Developers-Office365-Windows-CompliantDevice-Enforce
# Policy Persona Target : Users-External-Developers
# -------------------------------------------------------------------------------------------------------------------------------------
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA352-Users-External-Developers-Office365-Windows-CompliantDevice-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA352-Users-External-Developers-Office365-Windows-CompliantDevice-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA352-Users-External-Developers-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA352-Users-External-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
                   -GroupType DynamicMembership `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -startsWith "Internal_User") and (user.extensionAttribute8 -contains "_pilot1")' `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA352-Users-External-Developers-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA352-Users-External-Developers-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA352-Users-External-Developers" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA352-Users-External-Developers-Pilot1-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
        foreach ($pilot in 1..3) {
            ...
            EntraGroup -DisplayName "Entra-CA-Users-External-Developers-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-Users-External-Developers" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
                       -SecurityEnabled $true `
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA352-Users-External-Developers-Office365-Windows-CompliantDevice-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA352-Initial" `
                  -DisplayName "CA352-Initial-Users-External-Developers-Office365-Windows-CompliantDevice-Enforce" `
                  ...
                  -Cond_Platforms_includePlatforms @("All") `
                  -Cond_Platforms_excludePlatforms @("android", "iOS", "macOS", "linux") `
                  -GC_Operator "OR" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA352-Users-External-Developers-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA352-Users-External-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA352-Initial", "CA352-Pilot1", "CA352-Pilot2", "CA352-Pilot3", "CA352-Prod") `
                  -DisplayName "CA352-Pilot1-Users-External-Developers-Office365-Windows-CompliantDevice-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA352-Users-External-Developers-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -startsWith "Internal_User") and ((user.extensionAttribute8 -contains "_pilot1") or (user.extensionAttribute8 -contains "_pilot2"))' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA352-Users-External-Developers-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA352-Users-External-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA352-Users-External-Developers-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA352-Initial", "CA352-Pilot1", "CA352-Pilot2", "CA352-Pilot3", "CA352-Prod") `
                  -DisplayName "CA352-Pilot2-Users-External-Developers-Office365-Windows-CompliantDevice-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA352-Users-External-Developers-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -startsWith "Internal_User") and ((user.extensionAttribute8 -contains "_pilot1") or (user.extensionAttribute8 -contains "_pilot2") or (user.extensionAttribute8 -contains "_pilot3"))' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA352-Users-External-Developers-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA352-Users-External-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA352-Users-External-Developers-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA352-Initial", "CA352-Pilot1", "CA352-Pilot2", "CA352-Pilot3", "CA352-Prod") `
                  -DisplayName "CA352-Pilot3-Users-External-Developers-Office365s-Windows-CompliantDevice-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        $Users_Internal_All            = $EntraGroupsHashTable["Entra-CA-Users-External-Developers-All-Dynamic"]
        $Users_Internal_MFA_All        = $EntraGroupsHashTable["Entra-CA-Users-External-Developers-Req-MFA-All-Dynamic"]
        ...
        EntraGroup -DisplayName "Entra-CA-CA352-Users-External-Developers-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    if ( ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") -or ($Group_Targeting_Method -eq "Manual_Assignment_Simple") ) {
        ...
        $Users_Internal_All            = $EntraGroupsHashTable["Entra-CA-Users-External-Developers-All-Assigned"]
        $Users_Internal_MFA_All        = $EntraGroupsHashTable["Entra-CA-Users-External-Developers-Req-MFA-All-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA352-Initial", "CA352-Pilot1", "CA352-Pilot2", "CA352-Pilot3", "CA352-Prod") `
                  -DisplayName "CA352-Prod-Users-External-Developers-Office365-Windows-CompliantDevice-Enforce" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA352-$($PolicyVersion)" `
                  -DisplayName "CA352-$($PolicyVersion)-Users-External-Developers-Office365-Windows-CompliantDevice-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA352-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls "compliantDevice" `
                  -CreateUpdate
