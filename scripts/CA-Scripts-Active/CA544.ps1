<#
.SYNOPSIS
    CA544 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA544
# Policy Name           : CA544-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-FIDO-Allow
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA544-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-FIDO-Allow-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA544-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-FIDO-Allow" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA544-Teams-Rooms-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA544-Teams-Rooms" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
                   -GroupType DynamicMembership `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Teams_Room_Req_FIDO_Pilot1"])' `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA544-Teams-Rooms-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA544-Teams-Rooms-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA544-Teams-Rooms" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA544-Teams-Rooms-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA544-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-FIDO-Allow-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA544-Initial" `
                  -DisplayName "CA544-Initial-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-FIDO-Allow" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA544-Teams-Rooms-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA544-Teams-Rooms" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA544-Initial", "CA544-Pilot1", "CA544-Pilot2", "CA544-Pilot3", "CA544-Prod") `
                  -DisplayName "CA544-Pilot1-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-FIDO-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA544-Teams-Rooms-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Teams_Room_Req_FIDO_Pilot1","Teams_Room_Req_FIDO_Pilot2"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA544-Teams-Rooms-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA544-Teams-Rooms" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA544-Teams-Rooms-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA544-Initial", "CA544-Pilot1", "CA544-Pilot2", "CA544-Pilot3", "CA544-Prod") `
                  -DisplayName "CA544-Pilot2-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-FIDO-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA544-Teams-Rooms-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Teams_Room_Req_FIDO_Pilot1","Teams_Room_Req_FIDO_Pilot2","Teams_Room_Req_FIDO_Pilot3"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA544-Teams-Rooms-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA544-Teams-Rooms" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA544-Teams-Rooms-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA544-Initial", "CA544-Pilot1", "CA544-Pilot2", "CA544-Pilot3", "CA544-Prod") `
                  -DisplayName "CA544-Pilot3-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-FIDO-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA544-Teams-Rooms-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA544-Initial", "CA544-Pilot1", "CA544-Pilot2", "CA544-Pilot3", "CA544-Prod") `
                  -DisplayName "CA544-Prod-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-FIDO-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($Teams_Rooms_FIDO_All.id)) `
                  -State enabled `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA544-$($PolicyVersion)" `
                  -DisplayName "CA544-$($PolicyVersion)-Teams-Rooms-RegisterSecurityInfo-TrustedLocations-FIDO-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA544-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls @() `
                  -GC_authenticationStrength "FIDO Security Key & TAP" `
                  -CreateUpdate
