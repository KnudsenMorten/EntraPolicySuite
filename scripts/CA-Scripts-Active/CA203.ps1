<#
.SYNOPSIS
    CA203 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA203
# Policy Name           : CA203-Users-Internal-Office365-MacOS-CompliantDevice-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA203-Users-Internal-Office365-MacOS-CompliantDevice-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA203-Users-Internal-Office365-MacOS-CompliantDevice-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA203-Users-Internal-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA203-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA203-Users-Internal-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA203-Users-Internal-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA203-Users-Internal" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA203-Users-Internal-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA203-Users-Internal-Office365-MacOS-CompliantDevice-Enforce-Excluded-Assigned"]
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA203-Users-Internal-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA203-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA203-Initial", "CA203-Pilot1", "CA203-Pilot2", "CA203-Pilot3", "CA203-Prod") `
                  -DisplayName "CA203-Pilot1-Users-Internal-Office365-MacOS-CompliantDevice-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA203-Users-Internal-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA203-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA203-Users-Internal-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA203-Initial", "CA203-Pilot1", "CA203-Pilot2", "CA203-Pilot3", "CA203-Prod") `
                  -DisplayName "CA203-Pilot2-Users-Internal-Office365-MacOS-CompliantDevice-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA203-Users-Internal-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA203-Users-Internal" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA203-Users-Internal-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA203-Initial", "CA203-Pilot1", "CA203-Pilot2", "CA203-Pilot3", "CA203-Prod") `
                  -DisplayName "CA203-Pilot3-Users-Internal-Office365-MacOS-CompliantDevice-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA203-Users-Internal-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA203-Initial", "CA203-Pilot1", "CA203-Pilot2", "CA203-Pilot3", "CA203-Prod") `
                  -DisplayName "CA203-Prod-Users-Internal-Office365-MacOS-CompliantDevice-Enforce" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA203-$($PolicyVersion)" `
                  -DisplayName "CA203-$($PolicyVersion)-Users-Internal-CA203-Users-Internal-Office365-MacOS-CompliantDevice-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA203-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls "compliantDevice" `
                  -CreateUpdate
