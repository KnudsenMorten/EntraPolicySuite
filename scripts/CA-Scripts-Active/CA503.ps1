<#
.SYNOPSIS
    CA503 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA503
# Policy Name           : CA503-Shared-Device-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA503-Shared-Device-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA503-Shared-Device-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA503-Shared-Device-Users-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA503-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
                   -GroupType DynamicMembership `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Shared_Device_User_Req_Pwd_Pilot1"])' `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA503-Shared-Device-Users-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA503-Shared-Device-Users-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA503-Shared-Device-Users" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA503-Shared-Device-Users-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA503-Shared-Device-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA503-Initial" `
                  -DisplayName "CA503-Initial-Shared-Device-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA503-Shared-Device-Users-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA503-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA503-Initial", "CA503-Pilot1", "CA503-Pilot2", "CA503-Pilot3", "CA503-Prod") `
                  -DisplayName "CA503-Pilot1-Shared-Device-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA503-Shared-Device-Users-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Shared_Device_User_Req_Pwd_Pilot1","Shared_Device_User_Req_Pwd_Pilot2"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA503-Shared-Device-Users-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA503-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA503-Shared-Device-Users-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA503-Initial", "CA503-Pilot1", "CA503-Pilot2", "CA503-Pilot3", "CA503-Prod") `
                  -DisplayName "CA503-Pilot2-Shared-Device-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA503-Shared-Device-Users-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Shared_Device_User_Req_Pwd_Pilot1","Shared_Device_User_Req_Pwd_Pilot2","Shared_Device_User_Req_Pwd_Pilot3"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA503-Shared-Device-Users-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA503-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA503-Shared-Device-Users-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA503-Initial", "CA503-Pilot1", "CA503-Pilot2", "CA503-Pilot3", "CA503-Prod") `
                  -DisplayName "CA503-Pilot3-Shared-Device-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA503-Shared-Device-Users-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA503-Initial", "CA503-Pilot1", "CA503-Pilot2", "CA503-Pilot3", "CA503-Prod") `
                  -DisplayName "CA503-Prod-Shared-Device-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($Shared_Device_Users_Pwd_All.id)) `
                  -State enabled `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA503-$($PolicyVersion)" `
                  -DisplayName "CA503-$($PolicyVersion)-Shared-Device-Users-RegisterSecurityInfo-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA503-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls "mfa" `
                  -CreateUpdate
