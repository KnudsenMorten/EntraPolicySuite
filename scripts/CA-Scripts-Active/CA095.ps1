<#
.SYNOPSIS
    CA095 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA095
# Policy Name           : CA095-BreakGlassAccounts-MFA-AllApps-AnyPlatform-NonTrustedLocations-Block
# Policy Persona Target : BreakGlassAccounts
# -------------------------------------------------------------------------------------------------------------------------------------
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA095-BreakGlassAccounts-MFA-AllApps-AnyPlatform-NonTrustedLocations-Block-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA095-BreakGlassAccounts-MFA-AllApps-AnyPlatform-NonTrustedLocations-Block" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA095-BreakGlassAccounts-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA095-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
                   -GroupType DynamicMembership `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Break_Glass_Account_Req_MFA_Pilot1"])' `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA095-BreakGlassAccounts-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA095-BreakGlassAccounts-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA095-BreakGlassAccounts" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA095-BreakGlassAccounts-Pilot1-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-BreakGlassAccounts-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-BreakGlassAccounts" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
                       -SecurityEnabled $true `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-BreakGlassAccounts-Pilot1-Assigned"]
    }

    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA095-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA095-Initial" `
                  -DisplayName "CA095-Initial-BreakGlassAccounts-MFA-AllApps-AnyPlatform-NonTrustedLocations-Block" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA095-BreakGlassAccounts-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA095-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-BreakGlassAccounts-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA095-Initial", "CA095-Pilot1", "CA095-Pilot2", "CA095-Pilot3", "CA095-Prod") `
                  -DisplayName "CA095-Pilot1-BreakGlassAccounts-MFA-AllApps-AnyPlatform-NonTrustedLocations-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA095-BreakGlassAccounts-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Break_Glass_Account_Req_MFA_Pilot1","Break_Glass_Account_Req_MFA_Pilot2"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA095-BreakGlassAccounts-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA095-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA095-BreakGlassAccounts-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {

        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-BreakGlassAccounts-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-BreakGlassAccounts-Pilot2-Assigned"]
    }

    # Change to pilot 2 configuration, look for any policies using prefix array
    EntraCAPolicy -CAPolicyPrefixArray @("CA095-Initial", "CA095-Pilot1", "CA095-Pilot2", "CA095-Pilot3", "CA095-Prod") `
                  -DisplayName "CA095-Pilot2-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA095-BreakGlassAccounts-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Break_Glass_Account_Req_MFA_Pilot1","Break_Glass_Account_Req_MFA_Pilot2","Break_Glass_Account_Req_MFA_Pilot3"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA095-BreakGlassAccounts-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA095-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA095-BreakGlassAccounts-Pilot3-Assigned"]

    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {

        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-BreakGlassAccounts-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-BreakGlassAccounts-Pilot3-Assigned"]
    }

    # Change to pilot 3 configuration, look for any policies using prefix array
    EntraCAPolicy -CAPolicyPrefixArray @("CA095-Initial", "CA095-Pilot1", "CA095-Pilot2", "CA095-Pilot3", "CA095-Prod") `
                  -DisplayName "CA095-Pilot3-BreakGlassAccounts-MFA-AllApps-AnyPlatform-NonTrustedLocations-Block" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA095-BreakGlassAccounts-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA095-Initial", "CA095-Pilot1", "CA095-Pilot2", "CA095-Pilot3", "CA095-Prod") `
                  -DisplayName "CA095-Prod-BreakGlassAccounts-MFA-AllApps-AnyPlatform-NonTrustedLocations-Block" `
                  -Cond_Users_IncludeUsers @(($BreakGlassAccounts_MFA.id)) `
                  -Cond_Users_IncludeGroups @(($BreakGlassAccounts_MFA_All.id)) `
                  -Cond_Users_ExcludeUsers @() `
                  -Cond_Users_ExcludeGroups @() `
                  -State enabled `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA095-$($PolicyVersion)" `
                  -DisplayName "CA095-$($PolicyVersion)-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA095-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls "mfa" `
                  -CreateUpdate
...
elseif ($Mode -eq "GroupForceUpdate") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA095-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA095-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow" `
               -EntraGroupsHashTable $EntraGroupsHashTable `
               -AutomaticMailNickName `
               -MailEnabled $false `
               -SecurityEnabled $true `
