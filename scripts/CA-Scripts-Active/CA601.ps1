<#
.SYNOPSIS
    CA601 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA601
# Policy Name           : CA601-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-Pwd-Allow
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA601-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-Pwd-Allow-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA601-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-Pwd-Allow" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA601-ServiceAccounts-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA601-ServiceAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
                   -GroupType DynamicMembership `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Service_Account_Req_Pwd_Pilot1"])' `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA601-ServiceAccounts-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA601-ServiceAccounts-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA601-ServiceAccounts" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA601-ServiceAccounts-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA601-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-Pwd-Allow-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA601-Initial" `
                  -DisplayName "CA601-Initial-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-Pwd-Allow" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA601-ServiceAccounts-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA601-ServiceAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA601-Initial", "CA601-Pilot1", "CA601-Pilot2", "CA601-Pilot3", "CA601-Prod") `
                  -DisplayName "CA601-Pilot1-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA601-ServiceAccounts-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Service_Account_Req_Pwd_Pilot1","Service_Account_Req_Pwd_Pilot2"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA601-ServiceAccounts-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA601-ServiceAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA601-ServiceAccounts-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA601-Initial", "CA601-Pilot1", "CA601-Pilot2", "CA601-Pilot3", "CA601-Prod") `
                  -DisplayName "CA601-Pilot2-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA601-ServiceAccounts-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -in ["Service_Account_Req_Pwd_Pilot1","Service_Account_Req_Pwd_Pilot2","Service_Account_Req_Pwd_Pilot3"])' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA601-ServiceAccounts-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA601-ServiceAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA601-ServiceAccounts-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA601-Initial", "CA601-Pilot1", "CA601-Pilot2", "CA601-Pilot3", "CA601-Prod") `
                  -DisplayName "CA601-Pilot3-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA601-ServiceAccounts-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA601-Initial", "CA601-Pilot1", "CA601-Pilot2", "CA601-Pilot3", "CA601-Prod") `
                  -DisplayName "CA601-Prod-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($ServiceAccounts_Pwd_All.id)) `
                  -State enabled `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA601-$($PolicyVersion)" `
                  -DisplayName "CA601-$($PolicyVersion)-ServiceAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-Pwd-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA601-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls "mfa" `
                  -CreateUpdate
