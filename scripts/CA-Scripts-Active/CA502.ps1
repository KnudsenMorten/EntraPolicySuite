<#
.SYNOPSIS
    CA502 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA502
# Policy Name           : CA502-Shared-Device-Users-AllApps-AnyPlatform-PersistentBrowser-Enforce
# Policy Persona Target : Shared-Device-Users
# -------------------------------------------------------------------------------------------------------------------------------------
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA502-Shared-Device-Users-AllApps-AnyPlatform-PersistentBrowser-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA502-Shared-Device-Users-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA502-Shared-Device-Users-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA502-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
                   -GroupType DynamicMembership `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -startsWith "Shared_Device_User") and (user.extensionAttribute8 -contains "_pilot1")' `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA502-Shared-Device-Users-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA502-Shared-Device-Users-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA502-Shared-Device-Users" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA502-Shared-Device-Users-Pilot1-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-Shared-Device-Users-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-Shared-Device-Users" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
                       -SecurityEnabled $true `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-Shared-Device-Users-Pilot1-Assigned"]
    }

    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA502-Shared-Device-Users-AllApps-AnyPlatform-PersistentBrowser-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA502-Initial" `
                  -DisplayName "CA502-Initial-Shared-Device-Users-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA502-Shared-Device-Users-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA502-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-Shared-Device-Users-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA502-Initial", "CA502-Pilot1", "CA502-Pilot2", "CA502-Pilot3", "CA502-Prod") `
                  -DisplayName "CA502-Pilot1-Shared-Device-Users-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA502-Shared-Device-Users-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -startsWith "Shared_Device_User") and ((user.extensionAttribute8 -contains "_pilot1") or (user.extensionAttribute8 -contains "_pilot2"))' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA502-Shared-Device-Users-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA502-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA502-Shared-Device-Users-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {

        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-Shared-Device-Users-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-Shared-Device-Users-Pilot2-Assigned"]
    }

    # Change to pilot 2 configuration, look for any policies using prefix array
    EntraCAPolicy -CAPolicyPrefixArray @("CA502-Initial", "CA502-Pilot1", "CA502-Pilot2", "CA502-Pilot3", "CA502-Prod") `
                  -DisplayName "CA502-Pilot2-Shared-Device-Users-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA502-Shared-Device-Users-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -startsWith "Shared_Device_User") and ((user.extensionAttribute8 -contains "_pilot1") or (user.extensionAttribute8 -contains "_pilot2") or (user.extensionAttribute8 -contains "_pilot3"))' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA502-Shared-Device-Users-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA502-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA502-Shared-Device-Users-Pilot3-Assigned"]

    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {

        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-Shared-Device-Users-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-Shared-Device-Users-Pilot3-Assigned"]
    }

    # Change to pilot 3 configuration, look for any policies using prefix array
    EntraCAPolicy -CAPolicyPrefixArray @("CA502-Initial", "CA502-Pilot1", "CA502-Pilot2", "CA502-Pilot3", "CA502-Prod") `
                  -DisplayName "CA502-Pilot3-Shared-Device-Users-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA502-Shared-Device-Users-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA502-Initial", "CA502-Pilot1", "CA502-Pilot2", "CA502-Pilot3", "CA502-Prod") `
                  -DisplayName "CA502-Prod-Shared-Device-Users-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($Shared_Device_Users_All.id)) `
                  -State enabled `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA502-$($PolicyVersion)" `
                  -DisplayName "CA502-$($PolicyVersion)-Shared-Device-Users-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA502-Prod" `
                  -SC_persistentBrowser_Mode "never" `
                  -SC_persistentBrowser_isEnabled $true `
                  -CreateUpdate
