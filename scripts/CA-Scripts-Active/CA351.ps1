<#
.SYNOPSIS
    CA351 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA351
# Policy Name           : CA351-Users-External-Developers-AllApps-AnyPlatform-PersistentBrowser-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA351-Users-External-Developers-AllApps-AnyPlatform-PersistentBrowser-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA351-Users-External-Developers-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA351-Users-External-Developers-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA351-Users-External-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA351-Users-External-Developers-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA351-Users-External-Developers-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA351-Users-External-Developers" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA351-Users-External-Developers-Pilot1-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
        foreach ($pilot in 1..3) {
            ...
            EntraGroup -DisplayName "Entra-CA-Users-External-Developers-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-Externals" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
                       -SecurityEnabled $true `
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA351-Users-External-Developers-AllApps-AnyPlatform-PersistentBrowser-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA351-Initial" `
                  -DisplayName "CA351-Initial-Users-External-Developers-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA351-Users-External-Developers-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA351-Users-External-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA351-Initial", "CA351-Pilot1", "CA351-Pilot2", "CA351-Pilot3", "CA351-Prod") `
                  -DisplayName "CA351-Pilot1-Users-External-Developers-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA351-Users-External-Developers-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA351-Users-External-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA351-Users-External-Developers-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA351-Initial", "CA351-Pilot1", "CA351-Pilot2", "CA351-Pilot3", "CA351-Prod") `
                  -DisplayName "CA351-Pilot2-Users-External-Developers-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA351-Users-External-Developers-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA351-Users-External-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA351-Users-External-Developers-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA351-Initial", "CA351-Pilot1", "CA351-Pilot2", "CA351-Pilot3", "CA351-Prod") `
                  -DisplayName "CA351-Pilot3-Users-External-Developers-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA351-Users-External-Developers-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA351-Initial", "CA351-Pilot1", "CA351-Pilot2", "CA351-Pilot3", "CA351-Prod") `
                  -DisplayName "CA351-Prod-Users-External-Developers-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA351-$($PolicyVersion)" `
                  -DisplayName "CA351-$($PolicyVersion)-Users-External-Developers-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA351-Prod" `
                  -SC_persistentBrowser_Mode "never" `
                  -SC_persistentBrowser_isEnabled $true `
                  -CreateUpdate
