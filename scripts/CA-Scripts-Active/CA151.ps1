<#
.SYNOPSIS
    CA151 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA151
# Policy Name           : CA151-Admins-External-AllApps-AnyPlatform-PersistentBrowser-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA151-Admins-External-AllApps-AnyPlatform-PersistentBrowser-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA151-Admins-External-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA151-Admins-External-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA151-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA151-Admins-External-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA151-Admins-External-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA151-Admins" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA151-Admins-External-Pilot1-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-Admins-External-Pilot$pilot-Assigned" `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-Admins-External-Pilot1-Assigned"]
    }

    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA151-Admins-External-AllApps-AnyPlatform-PersistentBrowser-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA151-Initial" `
                  -DisplayName "CA151-Initial-Admins-External-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA151-Admins-External-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA151-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-Admins-External-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA151-Initial", "CA151-Pilot1", "CA151-Pilot2", "CA151-Pilot3", "CA151-Prod") `
                  -DisplayName "CA151-Pilot1-Admins-External-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA151-Admins-External-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA151-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA151-Admins-External-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {

        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-Admins-External-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-Admins-External-Pilot2-Assigned"]
    }

    # Change to pilot 2 configuration, look for any policies using prefix array
    EntraCAPolicy -CAPolicyPrefixArray @("CA151-Initial", "CA151-Pilot1", "CA151-Pilot2", "CA151-Pilot3", "CA151-Prod") `
                  -DisplayName "CA151-Pilot2-Admins-External-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA151-Admins-External-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA151-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA151-Admins-External-Pilot3-Assigned"]

    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {

        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-Admins-External-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-Admins" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-Admins-External-Pilot3-Assigned"]
    }

    # Change to pilot 3 configuration, look for any policies using prefix array
    EntraCAPolicy -CAPolicyPrefixArray @("CA151-Initial", "CA151-Pilot1", "CA151-Pilot2", "CA151-Pilot3", "CA151-Prod") `
                  -DisplayName "CA151-Pilot3-Admins-External-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA151-Admins-External-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA151-Initial", "CA151-Pilot1", "CA151-Pilot2", "CA151-Pilot3", "CA151-Prod") `
                  -DisplayName "CA151-Prod-Admins-External-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($Admins_External_All.id)) `
                  -State enabled `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA151-$($PolicyVersion)" `
                  -DisplayName "CA151-$($PolicyVersion)-Admins-External-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA151-Prod" `
                  -SC_persistentBrowser_Mode "never" `
                  -SC_persistentBrowser_isEnabled $true `
                  -CreateUpdate
