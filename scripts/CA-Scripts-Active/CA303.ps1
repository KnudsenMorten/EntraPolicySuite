<#
.SYNOPSIS
    CA303 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA303
# Policy Name           : CA303-Users-External-AllApps-AnyPlatform-PersistentBrowser-Enforce
# Policy Persona Target : Users-Externals
# -------------------------------------------------------------------------------------------------------------------------------------
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA303-Users-External-AllApps-AnyPlatform-PersistentBrowser-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA303-Users-External-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA303-Users-External-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA303-Users-External" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
                   -GroupType DynamicMembership `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -startsWith "External_User") and (user.extensionAttribute8 -contains "_pilot1")' `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA303-Users-External-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA303-Users-External-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA303-Users-External" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA303-Users-External-Pilot1-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-Users-External-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-Users-External" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
                       -SecurityEnabled $true `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-Users-External-Pilot1-Assigned"]
    }

    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA303-Users-External-AllApps-AnyPlatform-PersistentBrowser-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA303-Initial" `
                  -DisplayName "CA303-Initial-Users-External-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA303-Users-External-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA303-Users-External" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-Users-External-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-Users-External" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA303-Initial", "CA303-Pilot1", "CA303-Pilot2", "CA303-Pilot3", "CA303-Prod") `
                  -DisplayName "CA303-Pilot1-Users-External-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA303-Users-External-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -startsWith "External_User") and ((user.extensionAttribute8 -contains "_pilot1") or (user.extensionAttribute8 -contains "_pilot2"))' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA303-Users-External-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA303-Users-External" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA303-Users-External-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {

        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-Users-External-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-Users-External" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-Users-External-Pilot2-Assigned"]
    }

    # Change to pilot 2 configuration, look for any policies using prefix array
    EntraCAPolicy -CAPolicyPrefixArray @("CA303-Initial", "CA303-Pilot1", "CA303-Pilot2", "CA303-Pilot3", "CA303-Prod") `
                  -DisplayName "CA303-Pilot2-Users-External-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA303-Users-External-Pilot-Dynamic" `
                   -GroupQuery '(user.accountEnabled -eq true) and (user.extensionAttribute8 -startsWith "External_User") and ((user.extensionAttribute8 -contains "_pilot1") or (user.extensionAttribute8 -contains "_pilot2") or (user.extensionAttribute8 -contains "_pilot3"))' `
                   -MembershipRuleProcessingState On `
                   -ForceUpdate
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA303-Users-External-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA303-Users-External" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA303-Users-External-Pilot3-Assigned"]

    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {

        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-Users-External-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-Users-External" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-Users-External-Pilot3-Assigned"]
    }

    # Change to pilot 3 configuration, look for any policies using prefix array
    EntraCAPolicy -CAPolicyPrefixArray @("CA303-Initial", "CA303-Pilot1", "CA303-Pilot2", "CA303-Pilot3", "CA303-Prod") `
                  -DisplayName "CA303-Pilot3-Users-External-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA303-Users-External-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA303-Initial", "CA303-Pilot1", "CA303-Pilot2", "CA303-Pilot3", "CA303-Prod") `
                  -DisplayName "CA303-Prod-Users-External-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($Users_External_All.id)) `
                  -State enabled `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA303-$($PolicyVersion)" `
                  -DisplayName "CA303-$($PolicyVersion)-Users-External-AllApps-AnyPlatform-PersistentBrowser-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA303-Prod" `
                  -SC_persistentBrowser_Mode "never" `
                  -SC_persistentBrowser_isEnabled $true `
                  -CreateUpdate
