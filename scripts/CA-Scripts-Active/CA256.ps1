<#
.SYNOPSIS
    CA256 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA256
# Policy Name           : CA256-Users-Internal-Developers-MicrosoftIntuneEnrollment-AnyPlatform-MFA-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA256-Users-Internal-Developers-MicrosoftIntuneEnrollment-AnyPlatform-MFA-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA256-Users-Internal-Developers-MicrosoftIntuneEnrollment-AnyPlatform-MFA-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA256-Users-Internal-Developers-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA256-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA256-Users-Internal-Developers-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA256-Users-Internal-Developers-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA256-Users-Internal-Developers" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA256-Users-Internal-Developers-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA256-Users-Internal-Developers-MicrosoftIntuneEnrollment-AnyPlatform-MFA-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA256-Initial" `
                  -DisplayName "CA256-Initial-Users-Internal-Developers-MicrosoftIntuneEnrollment-AnyPlatform-MFA-Enforce" `
                  ...
                  -Cond_App_includeApplications @("d4ebce55-015a-49b5-a083-c84d1797ae8c") `
                  -GC_Operator "OR" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA256-Users-Internal-Developers-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA256-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA256-Initial", "CA256-Pilot1", "CA256-Pilot2", "CA256-Pilot3", "CA256-Prod") `
                  -DisplayName "CA256-Pilot1-Users-Internal-Developers-MicrosoftIntuneEnrollment-AnyPlatform-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA256-Users-Internal-Developers-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA256-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA256-Users-Internal-Developers-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA256-Initial", "CA256-Pilot1", "CA256-Pilot2", "CA256-Pilot3", "CA256-Prod") `
                  -DisplayName "CA256-Pilot2-Users-Internal-Developers-MicrosoftIntuneEnrollment-AnyPlatform-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA256-Users-Internal-Developers-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA256-Users-Internal-Developers" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA256-Users-Internal-Developers-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA256-Initial", "CA256-Pilot1", "CA256-Pilot2", "CA256-Pilot3", "CA256-Prod") `
                  -DisplayName "CA256-Pilot3-Users-Internal-Developers-MicrosoftIntuneEnrollment-AnyPlatform-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA256-Users-Internal-Developers-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA256-Initial", "CA256-Pilot1", "CA256-Pilot2", "CA256-Pilot3", "CA256-Prod") `
                  -DisplayName "CA256-Prod-Users-Internal-Developers-MicrosoftIntuneEnrollment-AnyPlatform-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($Users_Internal_Developers_MFA_All.id)) `
                  -State enabled `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA256-$($PolicyVersion)" `
                  -DisplayName "CA256-$($PolicyVersion)-Users-Internal-Developers-MicrosoftIntuneEnrollment-AnyPlatform-MFA-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA256-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls "mfa" `
                  -CreateUpdate
