<#
.SYNOPSIS
    CA508 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA508
# Policy Name           : CA508-Shared-Device-Users-AllApps-TrustedLocations-WHfB-Enforce
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA508-Shared-Device-Users-AllApps-TrustedLocations-WHfB-Enforce-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA508-Shared-Device-Users-AllApps-TrustedLocations-WHfB-Enforce" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA508-Shared-Device-Users-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA508-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA508-Shared-Device-Users-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA508-Shared-Device-Users-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA508-Shared-Device-Users" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA508-Shared-Device-Users-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA508-Shared-Device-Users-AllApps-TrustedLocations-WHfB-Enforce-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA508-Initial" `
                  -DisplayName "CA508-Initial-Shared-Device-Users-AllApps-TrustedLocations-WHfB-Enforce" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA508-Shared-Device-Users-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA508-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA508-Initial", "CA508-Pilot1", "CA508-Pilot2", "CA508-Pilot3", "CA508-Prod") `
                  -DisplayName "CA508-Pilot1-Shared-Device-Users-AllApps-TrustedLocations-WHfB-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA508-Shared-Device-Users-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA508-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA508-Shared-Device-Users-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA508-Initial", "CA508-Pilot1", "CA508-Pilot2", "CA508-Pilot3", "CA508-Prod") `
                  -DisplayName "CA508-Pilot2-Shared-Device-Users-AllApps-TrustedLocations-WHfB-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA508-Shared-Device-Users-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA508-Shared-Device-Users" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA508-Shared-Device-Users-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA508-Initial", "CA508-Pilot1", "CA508-Pilot2", "CA508-Pilot3", "CA508-Prod") `
                  -DisplayName "CA508-Pilot3-Shared-Device-Users-AllApps-TrustedLocations-WHfB-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA508-Shared-Device-Users-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA508-Initial", "CA508-Pilot1", "CA508-Pilot2", "CA508-Pilot3", "CA508-Prod") `
                  -DisplayName "CA508-Prod-Shared-Device-Users-AllApps-TrustedLocations-WHfB-Enforce" `
                  -Cond_Users_IncludeUsers @() `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA508-$($PolicyVersion)" `
                  -DisplayName "CA508-$($PolicyVersion)-Shared-Device-Users-AllApps-TrustedLocations-WHfB-Enforce" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA508-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls @() `
                  -GC_authenticationStrength "WHfB + TAP" `
                  -CreateUpdate
