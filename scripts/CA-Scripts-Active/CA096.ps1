<#
.SYNOPSIS
    CA096 - engine script in the Entra-Policy-Suite solution.
...
########################################################################################################################################
# Policy Prefix         : CA096
# Policy Name           : CA096-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow
...
if ($Mode -eq "Initial_Setup") {
    ...
    EntraGroup -DisplayName "Entra-CA-CA096-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow-Excluded-Assigned" `
               -Description "Excluded Users for Policy CA096-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow" `
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA096-BreakGlassAccounts-Pilot-Dynamic" `
                   -Description "Pilot Users for Entra-CA-CA096-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
                   -SecurityEnabled $true `
        ...
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA096-BreakGlassAccounts-Pilot-Dynamic"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        foreach ($pilot in 1..3) {

            # Create pilot group, if missing
            EntraGroup -DisplayName "Entra-CA-CA096-BreakGlassAccounts-Pilot$pilot-Assigned" `
                       -Description "Pilot Users for Entra-CA-CA096-BreakGlassAccounts" `
                       -EntraGroupsHashTable $EntraGroupsHashTable `
                       -AutomaticMailNickName `
                       -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA096-BreakGlassAccounts-Pilot1-Assigned"]
    }
    ...
    # Find group info in hash table
    $PolicyExcludeGroup = $EntraGroupsHashTable["Entra-CA-CA096-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow-Excluded-Assigned"]

    # Configure initial policy - see policy documentation on https://github.com/KnudsenMorten/EntraPolicySuite
    EntraCAPolicy -CAPolicyPrefix "CA096-Initial" `
                  -DisplayName "CA096-Initial-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow" `
...
elseif ($Mode -eq "Pilot1") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA096-BreakGlassAccounts-Pilot1-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA096-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA096-Initial", "CA096-Pilot1", "CA096-Pilot2", "CA096-Pilot3", "CA096-Prod") `
                  -DisplayName "CA096-Pilot1-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
...
elseif ($Mode -eq "Pilot2") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA096-BreakGlassAccounts-Pilot2-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA096-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA096-BreakGlassAccounts-Pilot2-Assigned"]
    }
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Simple") {
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA096-Initial", "CA096-Pilot1", "CA096-Pilot2", "CA096-Pilot3", "CA096-Prod") `
                  -DisplayName "CA096-Pilot2-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -State enabled `
                  -CreateUpdate
...
elseif ($Mode -eq "Pilot3") {
    ...
    elseif ($Group_Targeting_Method -eq "Manual_Assignment_Advanced") {
        ...
        # Create pilot target group, if missing
        EntraGroup -DisplayName "Entra-CA-CA096-BreakGlassAccounts-Pilot3-Assigned" `
                   -Description "Pilot Users for Entra-CA-CA096-BreakGlassAccounts" `
                   -EntraGroupsHashTable $EntraGroupsHashTable `
                   -AutomaticMailNickName `
                   -MailEnabled $false `
        ...
        # Find group info in hash table
        $PolicyPilotGroup = $EntraGroupsHashTable["Entra-CA-CA096-BreakGlassAccounts-Pilot3-Assigned"]
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA096-Initial", "CA096-Pilot1", "CA096-Pilot2", "CA096-Pilot3", "CA096-Prod") `
                  -DisplayName "CA096-Pilot3-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -CreateUpdate
}
...
elseif ($Mode -eq "Prod") {
    ...
    if ($Group_Targeting_Method -eq "Dynamic_Using_Tags") {
        ...
        EntraGroup -DisplayName "Entra-CA-CA096-BreakGlassAccounts-Pilot-Dynamic" -GroupType DynamicMembership -MembershipRuleProcessingState Paused -ForceUpdate
    }
    ...
    EntraCAPolicy -CAPolicyPrefixArray @("CA096-Initial", "CA096-Pilot1", "CA096-Pilot2", "CA096-Pilot3", "CA096-Prod") `
                  -DisplayName "CA096-Prod-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow" `
...
elseif ($Mode -eq "Install_Latest_Policy_Disabled") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA096-$($PolicyVersion)" `
                  -DisplayName "CA096-$($PolicyVersion)-BreakGlassAccounts-RegisterSecurityInfo-AnyPlatform-TrustedLocations-MFA-Allow" `
                  -Cond_Users_IncludeUsers @() `
                  -Cond_Users_IncludeGroups @(($PolicyPilotGroup.id)) `
                  -Cond_Users_ExcludeGroups @(($BreakGlassAccountsGroup.id), ($PolicyExcludeGroup.id)) `
                  -Cond_Users_ExcludeUsers @(($BreakGlassAccounts.id)) `
...
elseif ($Mode -eq "Update_Prod_Policy_To_Latest") {
    ...
    EntraCAPolicy -CAPolicyPrefix "CA096-Prod" `
                  -GC_Operator "OR" `
                  -GC_BuiltInControls "mfa" `
                  -CreateUpdate
