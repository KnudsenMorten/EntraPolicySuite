<#
.SYNOPSIS
    Entra-ID-User-Tagging-Troubleshooting-PS7 - engine script in the Entra-Policy-Suite solution.

.NOTES
    Solution       : Entra-Policy-Suite
    File           : Entra-ID-User-Tagging-Troubleshooting-PS7.ps1
...
<#
    ...
    $EntraID_Users_ALL_Scoped = $EntraID_Users_ALL | where-Object { ($_.DisplayName -ne "On-Premises Directory Synchronization Service Account") -and `
                                                                    ($_.UserPrincipalName -like "amhpa@*") }
...
Start-Transcript -Path "$($global:PathScripts)\OUTPUT\IdentityTagging_debug-PS7.log"
