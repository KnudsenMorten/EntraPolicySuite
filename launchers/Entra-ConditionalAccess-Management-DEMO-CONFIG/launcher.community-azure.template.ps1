...
<#
.SYNOPSIS
    Community cloud-native launcher for Entra-Policy-Suite\Entra-ConditionalAccess-Management-DEMO-CONFIG.
.DESCRIPTION
...
Write-PlatformLog -Context $ctx -Event 'engine.start' -Message 'Entra-ConditionalAccess-Management-DEMO-CONFIG (community cloud) starting'
...
if (-not $engine) { throw "Launcher: engine 'Entra-ConditionalAccess-Management-DEMO-CONFIG.ps1' not found at $engineOwner\SCRIPTS or $engineOwner\scripts. Expected the launcher to live at <solroot>\LAUNCHERS\<engine>\ with a sibling SCRIPTS\ or scripts\ folder." }
    ...
    & $engine
    Write-PlatformLog -Context $ctx -Event 'engine.end' -Message 'Entra-ConditionalAccess-Management-DEMO-CONFIG (community cloud) completed'
}
catch {
    Write-PlatformLog -Context $ctx -Severity Error -Event 'engine.fail' -Message "Entra-ConditionalAccess-Management-DEMO-CONFIG FAILED: $_" -Data @{ exception = $_.ToString(); stack = $_.ScriptStackTrace }
    throw
}
finally {
